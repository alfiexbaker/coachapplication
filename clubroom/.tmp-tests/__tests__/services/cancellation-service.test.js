...
(0, node_test_1.describe)('cancellationService', () => {
    ...
    (0, node_test_1.describe)('cancelBooking', () => {
        (0, node_test_1.default)('creates a cancellation record', async () => {
            ...
            const record = await cancellation_service_1.cancellationService.cancelBooking(bookingId, 'parent', {
        ...
        (0, node_test_1.default)('records refund amount from refundCalculation', async () => {
            const record = await cancellation_service_1.cancellationService.cancelBooking(`bk_${rid()}`, 'coach', {
                reason: 'Weather',
                coachId: `c_${rid()}`,
                refundCalculation: {
                    originalAmount: 25,
    ...
    (0, node_test_1.describe)('getCancellationRecords', () => {
        (0, node_test_1.default)('returns all records when no filter', async () => {
            ...
            const records = await cancellation_service_1.cancellationService.getCancellationRecords();
        ...
        (0, node_test_1.default)('filters by coachId', async () => {
            ...
            const records = await cancellation_service_1.cancellationService.getCancellationRecords(coachId);
    ...
    (0, node_test_1.describe)('getCancellationByBooking', () => {
        (0, node_test_1.default)('returns record for known booking', async () => {
            ...
            const record = await cancellation_service_1.cancellationService.getCancellationByBooking(bookingId);
            strict_1.default.ok(record);
            strict_1.default.equal(record.bookingId, bookingId);
        });
        (0, node_test_1.default)('returns null for unknown booking', async () => {
            const record = await cancellation_service_1.cancellationService.getCancellationByBooking(`unknown_${rid()}`);
    ...
    (0, node_test_1.describe)('no-show tracking', () => {
        (0, node_test_1.default)('getNoShowCount returns 0 for new family', async () => {
            const count = await cancellation_service_1.cancellationService.getNoShowCount(`fam_${rid()}`);
            strict_1.default.equal(count, 0);
        });
        (0, node_test_1.default)('incrementNoShow increases count', async () => {
            const familyId = `fam_${rid()}`;
            await cancellation_service_1.cancellationService.incrementNoShow(familyId);
            await cancellation_service_1.cancellationService.incrementNoShow(familyId);
            const count = await cancellation_service_1.cancellationService.getNoShowCount(familyId);
        ...
        (0, node_test_1.default)('resetNoShowCount resets to 0', async () => {
            ...
            await cancellation_service_1.cancellationService.incrementNoShow(familyId);
            await cancellation_service_1.cancellationService.incrementNoShow(familyId);
            await cancellation_service_1.cancellationService.resetNoShowCount(familyId);
            const count = await cancellation_service_1.cancellationService.getNoShowCount(familyId);
            strict_1.default.equal(count, 0);
        });
    });
    ...
    (0, node_test_1.describe)('getCancellationStats', () => {
        (0, node_test_1.default)('returns stats for coach with records', async () => {
            ...
            const stats = await cancellation_service_1.cancellationService.getCancellationStats(coachId);
        ...
        (0, node_test_1.default)('returns zero stats for coach with no records', async () => {
            const stats = await cancellation_service_1.cancellationService.getCancellationStats(`none_${rid()}`);
