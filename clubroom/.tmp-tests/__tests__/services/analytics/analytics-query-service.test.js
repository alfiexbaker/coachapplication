...
(0, node_test_1.describe)('AnalyticsQueryService', () => {
    ...
    (0, node_test_1.describe)('getAthleteAnalytics', () => {
        (0, node_test_1.it)('should return analytics for known athlete', async () => {
            const analytics = await analytics_query_service_1.analyticsQueryService.getAthleteAnalytics('athlete_1', 'MONTH');
        ...
        (0, node_test_1.it)('should return default analytics for unknown athlete', async () => {
            ...
            const analytics = await analytics_query_service_1.analyticsQueryService.getAthleteAnalytics(athleteId, 'WEEK');
            strict_1.default.ok(analytics);
        ...
        (0, node_test_1.it)('should update period dynamically', async () => {
            const week = await analytics_query_service_1.analyticsQueryService.getAthleteAnalytics('athlete_1', 'WEEK');
            const year = await analytics_query_service_1.analyticsQueryService.getAthleteAnalytics('athlete_1', 'YEAR');
    ...
    (0, node_test_1.describe)('getSkillHistory', () => {
        (0, node_test_1.it)('should return all skills for athlete', async () => {
            const skills = await analytics_query_service_1.analyticsQueryService.getSkillHistory('athlete_1');
        ...
        (0, node_test_1.it)('should return empty array for unknown athlete', async () => {
            const athleteId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const skills = await analytics_query_service_1.analyticsQueryService.getSkillHistory(athleteId);
            strict_1.default.ok(Array.isArray(skills));
            strict_1.default.equal(skills.length, 0);
        });
        (0, node_test_1.it)('should filter by skill name when provided', async () => {
            const skills = await analytics_query_service_1.analyticsQueryService.getSkillHistory('athlete_1', 'Dribbling');
            strict_1.default.ok(Array.isArray(skills));
            if (skills.length > 0) {
    ...
    (0, node_test_1.describe)('getAthleteGoals', () => {
        (0, node_test_1.it)('should return all goals for athlete', async () => {
            const goals = await analytics_query_service_1.analyticsQueryService.getAthleteGoals('athlete_1');
        ...
        (0, node_test_1.it)('should return empty array for unknown athlete', async () => {
            const athleteId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const goals = await analytics_query_service_1.analyticsQueryService.getAthleteGoals(athleteId);
            strict_1.default.ok(Array.isArray(goals));
            strict_1.default.equal(goals.length, 0);
        });
        (0, node_test_1.it)('should filter by status when provided', async () => {
            const activeGoals = await analytics_query_service_1.analyticsQueryService.getAthleteGoals('athlete_1', 'ACTIVE');
    ...
    (0, node_test_1.describe)('getSkillComparison', () => {
        (0, node_test_1.it)('should return skill comparison data', async () => {
            const comparison = await analytics_query_service_1.analyticsQueryService.getSkillComparison('athlete_1');
        ...
        (0, node_test_1.it)('should return empty skills for unknown athlete', async () => {
            const athleteId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const comparison = await analytics_query_service_1.analyticsQueryService.getSkillComparison(athleteId);
            strict_1.default.ok(comparison);
            strict_1.default.ok(Array.isArray(comparison.skills));
        ...
        (0, node_test_1.it)('should match skills from athlete analytics', async () => {
            const analytics = await analytics_query_service_1.analyticsQueryService.getAthleteAnalytics('athlete_1');
            const comparison = await analytics_query_service_1.analyticsQueryService.getSkillComparison('athlete_1');
