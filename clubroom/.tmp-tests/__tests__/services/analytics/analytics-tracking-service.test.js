...
(0, node_test_1.describe)('AnalyticsTrackingService', () => {
    ...
    (0, node_test_1.describe)('createGoal', () => {
        (0, node_test_1.it)('should create new goal with required fields', async () => {
            const goal = expectOk(await analytics_tracking_service_1.analyticsTrackingService.createGoal({
                athleteId: 'athlete_1',
    ...
    (0, node_test_1.describe)('updateGoalProgress', () => {
        (0, node_test_1.it)('should return success on successful update', async () => {
            ...
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.updateGoalProgress(goal.id, 50));
            strict_1.default.equal(result.progress, 50);
        });
        ...
        (0, node_test_1.it)('should clamp progress between 0 and 100', async () => {
            ...
            const result1 = expectOk(await analytics_tracking_service_1.analyticsTrackingService.updateGoalProgress(goal.id, 150));
            strict_1.default.equal(result1.progress, 100);
            const result2 = expectOk(await analytics_tracking_service_1.analyticsTrackingService.updateGoalProgress(goal.id, -10));
            strict_1.default.equal(result2.progress, 0);
        });
        (0, node_test_1.it)('should mark goal as COMPLETED when progress reaches 100', async () => {
            ...
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.updateGoalProgress(goal.id, 100));
    ...
    (0, node_test_1.describe)('completeMilestone', () => {
        (0, node_test_1.it)('should return success and mark milestone as completed', async () => {
            ...
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.completeMilestone(goal.id, milestoneId));
        ...
        (0, node_test_1.it)('should recalculate progress based on completed milestones', async () => {
            ...
            expectOk(await analytics_tracking_service_1.analyticsTrackingService.completeMilestone(goal.id, goal.milestones[0].id));
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.completeMilestone(goal.id, goal.milestones[1].id));
            strict_1.default.equal(result.progress, 50);
        });
        (0, node_test_1.it)('should mark goal as COMPLETED when all milestones done', async () => {
            ...
            expectOk(await analytics_tracking_service_1.analyticsTrackingService.completeMilestone(goal.id, goal.milestones[0].id));
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.completeMilestone(goal.id, goal.milestones[1].id));
            strict_1.default.equal(result.progress, 100);
            strict_1.default.equal(result.status, 'COMPLETED');
        });
    });
    (0, node_test_1.describe)('addMilestone', () => {
        (0, node_test_1.it)('should return success and add milestone to goal', async () => {
            ...
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.addMilestone(goal.id, 'New milestone'));
            strict_1.default.equal(result.milestones.length, 1);
            strict_1.default.equal(result.milestones[0].title, 'New milestone');
        });
        (0, node_test_1.it)('should return error for non-existent goal', async () => {
            const fakeId = 'goal-fake-' + Math.random().toString(36).slice(2);
        ...
        (0, node_test_1.it)('should recalculate progress when milestone added', async () => {
            ...
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.addMilestone(goal.id, 'Step 3'));
            strict_1.default.equal(result.progress, 33);
        });
    });
    (0, node_test_1.describe)('abandonGoal', () => {
        (0, node_test_1.it)('should return success and mark goal as ABANDONED', async () => {
            ...
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.abandonGoal(goal.id));
            strict_1.default.equal(result.status, 'ABANDONED');
        });
        (0, node_test_1.it)('should return error for non-existent goal', async () => {
            const fakeId = 'goal-fake-' + Math.random().toString(36).slice(2);
        ...
        (0, node_test_1.it)('should preserve progress when abandoning', async () => {
            ...
            expectOk(await analytics_tracking_service_1.analyticsTrackingService.updateGoalProgress(goal.id, 75));
            const result = expectOk(await analytics_tracking_service_1.analyticsTrackingService.abandonGoal(goal.id));
            strict_1.default.equal(result.progress, 75);
            strict_1.default.equal(result.status, 'ABANDONED');
        });
    });
});
