...
(0, node_test_1.describe)('AnalyticsExportService', () => {
    ...
    (0, node_test_1.describe)('getCoachAnalytics', () => {
        (0, node_test_1.it)('should return analytics for known coach', async () => {
            const analytics = expectOk(await analytics_export_service_1.analyticsExportService.getCoachAnalytics('coach1', 'MONTH'));
        ...
        (0, node_test_1.it)('should return default analytics for unknown coach', async () => {
            ...
            const analytics = expectOk(await analytics_export_service_1.analyticsExportService.getCoachAnalytics(coachId, 'WEEK'));
            strict_1.default.ok(analytics);
        ...
        (0, node_test_1.it)('should update period and date range dynamically', async () => {
            const week = expectOk(await analytics_export_service_1.analyticsExportService.getCoachAnalytics('coach1', 'WEEK'));
            const month = expectOk(await analytics_export_service_1.analyticsExportService.getCoachAnalytics('coach1', 'MONTH'));
        ...
        (0, node_test_1.it)('should regenerate revenue chart for different periods', async () => {
            const weekAnalytics = expectOk(await analytics_export_service_1.analyticsExportService.getCoachAnalytics('coach1', 'WEEK'));
            const yearAnalytics = expectOk(await analytics_export_service_1.analyticsExportService.getCoachAnalytics('coach1', 'YEAR'));
    ...
    (0, node_test_1.describe)('getRevenueChart', () => {
        (0, node_test_1.it)('should return revenue data points', async () => {
            const chart = expectOk(await analytics_export_service_1.analyticsExportService.getRevenueChart('coach1', 'MONTH'));
        ...
        (0, node_test_1.it)('should return mock data for unknown coach', async () => {
            const coachId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const chart = expectOk(await analytics_export_service_1.analyticsExportService.getRevenueChart(coachId, 'WEEK'));
            strict_1.default.ok(Array.isArray(chart));
            strict_1.default.ok(chart.length > 0);
        });
        (0, node_test_1.it)('should vary data points by period', async () => {
            const weekChart = expectOk(await analytics_export_service_1.analyticsExportService.getRevenueChart('coach1', 'WEEK'));
            const yearChart = expectOk(await analytics_export_service_1.analyticsExportService.getRevenueChart('coach1', 'YEAR'));
    ...
    (0, node_test_1.describe)('getRetentionMetrics', () => {
        (0, node_test_1.it)('should return retention data for known coach', async () => {
            const retention = expectOk(await analytics_export_service_1.analyticsExportService.getRetentionMetrics('coach1'));
        ...
        (0, node_test_1.it)('should return default retention for unknown coach', async () => {
            const coachId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const retention = expectOk(await analytics_export_service_1.analyticsExportService.getRetentionMetrics(coachId));
            strict_1.default.ok(retention);
    ...
    (0, node_test_1.describe)('getCancellationPatterns', () => {
        (0, node_test_1.it)('should return cancellation stats for known coach', async () => {
            const stats = expectOk(await analytics_export_service_1.analyticsExportService.getCancellationPatterns('coach1'));
        ...
        (0, node_test_1.it)('should return empty stats for unknown coach', async () => {
            const coachId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const stats = expectOk(await analytics_export_service_1.analyticsExportService.getCancellationPatterns(coachId));
            strict_1.default.ok(stats);
    ...
    (0, node_test_1.describe)('getPeakHours', () => {
        (0, node_test_1.it)('should return peak hours heatmap data', async () => {
            const peakHours = expectOk(await analytics_export_service_1.analyticsExportService.getPeakHours('coach1'));
        ...
        (0, node_test_1.it)('should return generated peak hours for unknown coach', async () => {
            const coachId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const peakHours = expectOk(await analytics_export_service_1.analyticsExportService.getPeakHours(coachId));
            strict_1.default.ok(Array.isArray(peakHours));
            strict_1.default.ok(peakHours.length > 0);
        });
    });
    (0, node_test_1.describe)('getTopSkills', () => {
        (0, node_test_1.it)('should return top skills for known coach', async () => {
            const skills = expectOk(await analytics_export_service_1.analyticsExportService.getTopSkills('coach1'));
        ...
        (0, node_test_1.it)('should return empty array for unknown coach', async () => {
            const coachId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const skills = expectOk(await analytics_export_service_1.analyticsExportService.getTopSkills(coachId));
            strict_1.default.ok(Array.isArray(skills));
        });
    ...
    (0, node_test_1.describe)('getSessionStats', () => {
        (0, node_test_1.it)('should return session statistics for known coach', async () => {
            const stats = expectOk(await analytics_export_service_1.analyticsExportService.getSessionStats('coach1'));
            strict_1.default.ok(stats);
        ...
        (0, node_test_1.it)('should return default stats for unknown coach', async () => {
            const coachId = 'test-unknown-' + Math.random().toString(36).slice(2);
            const stats = expectOk(await analytics_export_service_1.analyticsExportService.getSessionStats(coachId));
            strict_1.default.ok(stats);
