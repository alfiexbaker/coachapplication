...
(0, node_test_1.describe)('coachService', () => {
    ...
    (0, node_test_1.describe)('getCoach', () => {
        ...
        (0, node_test_1.default)('accepts canonical POC coach id alias', async () => {
            const result = await coach_service_1.coachService.getCoach(poc_accounts_1.POC_ACCOUNT_IDS.coach);
            strict_1.default.equal(result.success, true);
            if (result.success) {
    ...
    (0, node_test_1.describe)('getCoachReviews', () => {
        ...
        (0, node_test_1.default)('accepts canonical POC coach id alias', async () => {
            const result = await coach_service_1.coachService.getCoachReviews(poc_accounts_1.POC_ACCOUNT_IDS.coach);
            strict_1.default.equal(result.success, true);
            if (result.success) {
                strict_1.default.ok(result.data.length > 0);
            }
    ...
    (0, node_test_1.describe)('submitReview', () => {
        (0, node_test_1.default)('creates a new review and returns ok', async () => {
            const result = await coach_service_1.coachService.submitReview(poc_accounts_1.POC_ACCOUNT_IDS.coach, {
                rating: 5,
            ...
            if (result.success) {
                ...
                strict_1.default.equal(result.data.coachId, poc_accounts_1.POC_ACCOUNT_IDS.coachStorage);
