...
const rescheduleService = {
    async createProposal(params) {
        ...
        await mockRescheduleRequested(params.coachName, new Date(params.originalDateTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), new Date(params.proposedDateTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), params.respondentId);
    ...
    async getProposalsForBooking(bookingId) {
        const proposals = await loadProposals();
        return proposals.filter((p) => p.bookingId === bookingId);
    },
    async getPendingProposals(userId) {
        const proposals = await loadProposals();
        return proposals.filter((p) => p.status === 'pending' && (p.initiatorId === userId || p.respondentId === userId));
    },
...
(0, node_test_1.describe)('RescheduleService', () => {
    ...
    (0, node_test_1.describe)('expireStaleProposals', () => {
        (0, node_test_1.it)('should expire proposals past their expiresAt', async () => {
            ...
            const count = await rescheduleService.expireStaleProposals();
        ...
        (0, node_test_1.it)('should only expire pending proposals, not already-resolved ones', async () => {
            ...
            await saveProposals(proposals);
            const count = await rescheduleService.expireStaleProposals();
            node_assert_1.default.strictEqual(count, 1); // Only the pending one
            const stored = await loadProposals();
            node_assert_1.default.strictEqual(stored[0].status, 'expired');
    ...
    (0, node_test_1.describe)('getProposalsForBooking', () => {
        (0, node_test_1.it)('should return proposals for a given booking', async () => {
            ...
            const results = await rescheduleService.getProposalsForBooking('booking_A');
            node_assert_1.default.strictEqual(results.length, 2);
            node_assert_1.default.ok(results.every((p) => p.bookingId === 'booking_A'));
        });
        (0, node_test_1.it)('should return empty array for unknown booking', async () => {
            const results = await rescheduleService.getProposalsForBooking('nonexistent');
    ...
    (0, node_test_1.describe)('getPendingProposals', () => {
        (0, node_test_1.it)('should return only pending proposals for a user', async () => {
            ...
            const pending = await rescheduleService.getPendingProposals('coach_1');
            node_assert_1.default.strictEqual(pending.length, 1);
            node_assert_1.default.strictEqual(pending[0].id, created2.data.id);
        });
        (0, node_test_1.it)('should include proposals where user is respondent', async () => {
            await rescheduleService.createProposal(makeCreateParams());
            // parent_1 is the respondent
            const pending = await rescheduleService.getPendingProposals('parent_1');
            node_assert_1.default.strictEqual(pending.length, 1);
        });
        (0, node_test_1.it)('should return empty array for user with no proposals', async () => {
            const pending = await rescheduleService.getPendingProposals('unknown_user');
            node_assert_1.default.deepStrictEqual(pending, []);
        });
    });
    // --------------------------------------------------------------------------
