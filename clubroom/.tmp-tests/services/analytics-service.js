...
async function loadAnalytics() {
    try {
        const stored = await async_storage_1.default.getItem(ANALYTICS_STORAGE_KEY);
        if (stored)
...
async function loadGoals() {
    try {
        const stored = await async_storage_1.default.getItem(GOALS_STORAGE_KEY);
        if (stored)
            return JSON.parse(stored);
    }
    catch (error) {
...
exports.analyticsService = {
    ...
    async updateSkillLevel(athleteId, skill, newLevel) {
        if (USE_MOCK) {
            ...
            await async_storage_1.default.setItem(ANALYTICS_STORAGE_KEY, JSON.stringify(analyticsCache));
        }
    },
    /**
 ...
 */
function generateMockRevenueChart(period, baseRevenue) {
    const points = [];
    const now = new Date();
    let dataPoints;
    let intervalDays;
...
let coachAnalyticsCache = { ...MOCK_COACH_ANALYTICS };
async function loadCoachAnalytics() {
    try {
        const stored = await async_storage_1.default.getItem(COACH_ANALYTICS_STORAGE_KEY);
        if (stored)
            return JSON.parse(stored);
    }
    catch (error) {
...
async function saveCoachAnalytics(analytics) {
    try {
