...
async function resolveAthleteName(athleteId, fallback) {
...
exports.eventInviteService = {
    ...
    async inviteSquadsToEvent(input) {
        ...
        for (const [parentId, athletes] of parentMap.entries()) {
            try {
                const athleteNames = (await Promise.all(athletes.map((athlete, index) => resolveAthleteName(athlete.athleteId, `Athlete ${index + 1}`)))).join(', ');
