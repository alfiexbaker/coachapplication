...
async function resolveAthleteNames(athleteIds, fallbackNames = []) {
    const uniqueAthleteIds = Array.from(new Set(athleteIds.filter(Boolean)));
    if (uniqueAthleteIds.length === 0) {
        return fallbackNames;
    }
    const usersResult = await user_service_1.userService.getUsersByIds(uniqueAthleteIds);
...
exports.sessionInviteService = {
    ...
    async _createSingleInvite(input) {
        ...
        if (USE_MOCK) {
            ...
            await invite_hold_service_1.inviteHoldService.createHolds(input.coachId, newInvite.id, input.proposedSlots.map((s) => ({
                date: s.date,
        ...
        if (USE_MOCK) {
            ...
            if (input.response === 'ACCEPTED') {
                ...
                const bookingResult = await booking_service_1.bookingService.createBooking({
                    ...
                    athleteNames: athleteNames.length > 0 ? athleteNames : invite.athleteIds.map(() => 'Athlete'),
                ...
                notification.body = `${parentName} accepted your invite for ${athleteDisplay}. Session confirmed for ${new Date(input.selectedSlot.date).toLocaleDateString('en-GB', {
                    weekday: 'short',
                    day: 'numeric',
                    month: 'short',
     ...
     */
    async sendInviteReminder(inviteId) {
        if (USE_MOCK) {
            invitesCache = await loadFromStorage();
            ...
            if (invite.status !== 'PENDING') {
                return (0, result_1.err)((0, result_1.serviceError)('VALIDATION', 'Only pending invites can be reminded.'));
            }
            if (new Date(invite.expiresAt) <= new Date()) {
                return (0, result_1.err)((0, result_1.serviceError)('VALIDATION', 'Invite has expired.'));
            }
            ...
            const reminderResult = await notification_service_1.notificationService.create({
                id: api_client_1.apiClient.generateId('notif'),
                type: 'reminder',
                notificationType: 'SESSION_INVITE',
            ...
            if (!reminderResult.success) {
                return (0, result_1.err)(reminderResult.error);
            }
    ...
    async getCoachInvites(coachId) {
        if (USE_MOCK) {
            invitesCache = await loadFromStorage();
            return invitesCache.filter((inv) => (0, account_id_1.accountIdsMatch)(inv.coachId, coachId));
        }
    ...
    async getParentInvites(parentId) {
        if (USE_MOCK) {
            invitesCache = await loadFromStorage();
            return invitesCache.filter((inv) => (0, account_id_1.accountIdsMatch)(inv.parentId, parentId) && !inv.dismissed);
        }
    ...
    async getOpenInvites() {
        if (USE_MOCK) {
            invitesCache = await loadFromStorage();
            return invitesCache.filter((inv) => (!inv.inviteType || inv.inviteType === 'OPEN') &&
                inv.status === 'PENDING' &&
    ...
    async getClosedInvitesForParent(parentId) {
        if (USE_MOCK) {
            invitesCache = await loadFromStorage();
            return invitesCache.filter((inv) => inv.inviteType === 'CLOSED' && (0, account_id_1.accountIdsMatch)(inv.parentId, parentId) && !inv.dismissed);
        }
    ...
    async getSquadOnlyInvitesForParent(parentId, memberSquadIds) {
        if (USE_MOCK) {
            ...
            return invitesCache.filter((inv) => {
                ...
                if ((0, account_id_1.accountIdsMatch)(inv.parentId, parentId))
                    return true;
        ...
        if (USE_MOCK) {
            ...
            return invitesCache.filter((inv) => {
                ...
                if (type === 'OPEN')
                    return true;
                if (type === 'CLOSED')
                    return (0, account_id_1.accountIdsMatch)(inv.parentId, parentId);
                if (type === 'SQUAD_ONLY') {
                    if ((0, account_id_1.accountIdsMatch)(inv.parentId, parentId))
                        return true;
                    if (inv.squadIds && inv.squadIds.some((sid) => memberSquadIds.includes(sid)))
