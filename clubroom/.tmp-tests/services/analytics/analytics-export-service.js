...
async function loadCoachAnalytics() {
    try {
        const stored = await api_client_1.apiClient.get(storage_keys_1.STORAGE_KEYS.COACH_ANALYTICS, null);
        if (stored)
            return stored;
    }
    catch (error) {
...
async function saveCoachAnalytics(analytics) {
    try {
        await api_client_1.apiClient.set(storage_keys_1.STORAGE_KEYS.COACH_ANALYTICS, analytics);
    }
    catch (error) {
