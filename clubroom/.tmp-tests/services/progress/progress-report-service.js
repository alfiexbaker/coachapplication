...
async function getAthleteProgress(athleteId, viewerRole = 'parent') {
    ...
    const improvementRate = skills.length > 0 ? Math.round((improvingSkills / skills.length) * 100) : 0;
    // Filter badges for visibility
    const visibleBadges = viewerRole === 'coach' ? badges : badges.filter((b) => b.visibility !== 'coach_only');
    return {
        athleteId,
