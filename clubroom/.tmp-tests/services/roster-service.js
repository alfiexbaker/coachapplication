...
async function loadRemovalHistory() {
    try {
        const stored = await async_storage_1.default.getItem(REMOVAL_HISTORY_KEY);
        if (stored)
...
async function saveRemovalHistory(history) {
    try {
        await async_storage_1.default.setItem(REMOVAL_HISTORY_KEY, JSON.stringify(history));
    }
    catch (error) {
...
async function saveToStorage(roster) {
    try {
        await async_storage_1.default.setItem(STORAGE_KEY, JSON.stringify(roster));
    }
    catch (error) {
