...
async function getAllDrills() {
    const drills = await storage_service_1.storageService.getItem(DRILLS_STORAGE_KEY, []);
    if (drills.length === 0) {
        return [...MOCK_DRILLS];
    }
    return drills;
...
async function saveDrills(drills) {
    await storage_service_1.storageService.setItem(DRILLS_STORAGE_KEY, drills);
}
/**
 * Get all assignments from storage
 */
async function getAllAssignments() {
    const assignments = await storage_service_1.storageService.getItem(ASSIGNMENTS_STORAGE_KEY, []);
...
async function saveAssignments(assignments) {
    await storage_service_1.storageService.setItem(ASSIGNMENTS_STORAGE_KEY, assignments);
}
/**
