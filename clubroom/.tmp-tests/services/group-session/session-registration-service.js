...
exports.sessionRegistrationService = {
     ...
     */
    async register(sessionId, athleteId, athleteName, parentId, parentName) {
        if (USE_MOCK) {
            ...
            if (!isFull) {
                await notification_trigger_1.notificationTriggers.groupRegistered(athleteName, session.title, session.coachId);
            }
    ...
    async cancelRegistration(registrationId) {
        if (USE_MOCK) {
            ...
            if (sessionForNotif && wasRegistered) {
                await notification_trigger_1.notificationTriggers.groupCancelledRegistration(registration.athleteName, sessionForNotif.title, sessionForNotif.coachId);
            }
            // Update session counts
