...
exports.getParentForAthlete = getParentForAthlete;
exports.getBookingsForCoach = getBookingsForCoach;
exports.getBookingsForAthlete = getBookingsForAthlete;
exports.getSessionsForCoach = getSessionsForCoach;
exports.getSessionsForAthlete = getSessionsForAthlete;
exports.getBadgeCatalog = getBadgeCatalog;
exports.getBadgeAwardsForAthlete = getBadgeAwardsForAthlete;
exports.getBadgeAwardsForSession = getBadgeAwardsForSession;
exports.getRecentBadgeAwards = getRecentBadgeAwards;
exports.getConversationsForUser = getConversationsForUser;
exports.getMessagesForConversation = getMessagesForConversation;
exports.getReviewsForCoach = getReviewsForCoach;
exports.getCommentsForPost = getCommentsForPost;
exports.getPostById = getPostById;
exports.getAllCoachesWithProfiles = getAllCoachesWithProfiles;
exports.getDistanceBetweenPostcodes = getDistanceBetweenPostcodes;
exports.formatGBP = formatGBP;
exports.formatTime = formatTime;
exports.formatDate = formatDate;
exports.formatDateTime = formatDateTime;
exports.getTemplatesForCoach = getTemplatesForCoach;
exports.getCoachSessionsForCoach = getCoachSessionsForCoach;
exports.getOpenSessions = getOpenSessions;
exports.getGuestAthletesForCoach = getGuestAthletesForCoach;
exports.getRequestsForSession = getRequestsForSession;
exports.getDirectoryForCoach = getDirectoryForCoach;
...
exports.getAggregatedFeed = getAggregatedFeed;
exports.getClubSquads = getClubSquads;
exports.getClubFeed = getClubFeed;
exports.addClubFeedPost = addClubFeedPost;
...
exports.MOCK_USERS = [
    ...
    // Users with children (can book on behalf of kids)
    {
        id: 'user4',
...
// ===== CONVERSATIONS =====
exports.MOCK_CONVERSATIONS = [
    {
        id: 'conv1',
        participants: ['coach1', 'user4'],
...
exports.MOCK_POSTS = [
    {
        ...
        likes: ['user1', 'user4', 'coach2', 'coach1', 'user2'],
        commentCount: 4,
    ...
    },
    {
        ...
        likes: ['user1', 'user2', 'user3', 'user4', 'user5', 'coach1', 'coach3'],
        commentCount: 6,
    ...
    },
    {
        ...
        likes: ['user1', 'user2', 'user4', 'coach3'],
        commentCount: 3,
    ...
    },
    {
        ...
        likes: ['user1', 'user2', 'coach1', 'coach2', 'coach3', 'user5'],
        commentCount: 5,
...
function getCoachProfile(userId) {
    return exports.MOCK_COACH_PROFILES.find((cp) => cp.userId === userId);
}
function getUserProfile(userId) {
    return exports.MOCK_USER_PROFILES.find((up) => up.userId === userId);
}
function getChildrenForParent(parentId) {
    const childIds = exports.MOCK_RELATIONSHIPS.filter((r) => r.userId === parentId).map((r) => r.childId);
    return exports.MOCK_USERS.filter((u) => childIds.includes(u.id));
}
function getParentForAthlete(athleteId) {
    const relationship = exports.MOCK_RELATIONSHIPS.find((r) => r.childId === athleteId);
    if (!relationship)
        return undefined;
    return exports.MOCK_USERS.find((u) => u.id === relationship.userId);
}
function getBookingsForCoach(coachId) {
    return exports.MOCK_BOOKINGS.filter((b) => b.coachId === coachId);
}
function getBookingsForAthlete(athleteId) {
    return exports.MOCK_BOOKINGS.filter((b) => b.athleteId === athleteId);
}
function getSessionsForCoach(coachId) {
    return exports.MOCK_SESSIONS.filter((s) => s.coachId === coachId);
}
function getSessionsForAthlete(athleteId) {
    return exports.MOCK_SESSIONS.filter((s) => s.athleteId === athleteId);
}
function getBadgeCatalog() {
    return exports.badgeCatalog;
}
function getBadgeAwardsForAthlete(athleteId) {
    return exports.badgeAwards
        .filter((award) => award.athleteId === athleteId)
        .sort((a, b) => new Date(b.awardedAt).getTime() - new Date(a.awardedAt).getTime());
}
function getBadgeAwardsForSession(sessionId) {
    return exports.badgeAwards
        .filter((award) => award.sessionId === sessionId)
        .sort((a, b) => new Date(b.awardedAt).getTime() - new Date(a.awardedAt).getTime());
}
function getRecentBadgeAwards(limit = 5) {
    return [...exports.badgeAwards]
        .sort((a, b) => new Date(b.awardedAt).getTime() - new Date(a.awardedAt).getTime())
        .slice(0, limit);
}
function getConversationsForUser(userId) {
    return exports.MOCK_CONVERSATIONS.filter((c) => c.participants.includes(userId));
}
function getMessagesForConversation(conversationId) {
    return exports.MOCK_MESSAGES.filter((m) => m.conversationId === conversationId);
}
function getReviewsForCoach(coachId) {
    return exports.MOCK_REVIEWS.filter((r) => r.coachId === coachId);
}
function getCommentsForPost(postId) {
    return exports.MOCK_COMMENTS.filter((c) => c.postId === postId);
}
function getPostById(postId) {
    return exports.MOCK_POSTS.find((p) => p.id === postId);
}
// Get all coaches with their profiles (for search/discover)
function getAllCoachesWithProfiles() {
    return exports.MOCK_USERS.filter((u) => u.role === 'COACH').map((coach) => ({
        ...coach,
...
// Session Templates
exports.MOCK_SESSION_TEMPLATES = [
    {
        id: 'template1',
        coachId: 'coach1',
...
exports.MOCK_ATHLETE_DIRECTORY = [
    {
        id: 'dir1',
        coachId: 'coach1',
        athleteId: 'user1',
...
// Session Recaps
exports.MOCK_SESSION_RECAPS = [
    {
        id: 'recap1',
        sessionId: 'session5',
        coachId: 'coach2',
        summary: 'Excellent session with James. Focused on weak foot finishing and he made tremendous progress. His confidence has grown significantly.',
...
function getTemplatesForCoach(coachId) {
    return exports.MOCK_SESSION_TEMPLATES.filter((t) => t.coachId === coachId);
}
function getCoachSessionsForCoach(coachId) {
    return exports.MOCK_COACH_SESSIONS.filter((s) => s.coachId === coachId);
}
function getOpenSessions() {
    return exports.MOCK_COACH_SESSIONS.filter((s) => s.state === 'OPEN' && s.isOpenToRequests);
}
function getGuestAthletesForCoach(coachId) {
    return exports.MOCK_GUEST_ATHLETES.filter((g) => g.coachId === coachId);
}
function getRequestsForSession(sessionId) {
    return exports.MOCK_SESSION_REQUESTS.filter((r) => r.sessionId === sessionId);
}
function getDirectoryForCoach(coachId) {
    return exports.MOCK_ATHLETE_DIRECTORY.filter((d) => d.coachId === coachId);
}
function getNotificationsForUser(userId) {
    return exports.MOCK_NOTIFICATIONS.filter((n) => n.userId === userId);
}
function getPaymentsForUser(userId) {
    return exports.MOCK_PAYMENTS.filter((p) => p.payerId === userId);
}
function getInviteCodesForCoach(coachId) {
    return exports.MOCK_INVITE_CODES.filter((i) => i.coachId === coachId);
}
// ===== CLUB HUB DATA =====
exports.clubs = [
    {
    ...
    },
    {
        ...
        squadCount: 6,
        ownerId: 'coach6',
        ownerName: 'Lucy Brown',
        inviteCode: 'NLU-FAMILY',
    },
];
exports.clubMemberships = [
    ...
    // North London United memberships
    {
        clubId: 'club_united',
        userId: 'coach6',
        role: 'OWNER',
...
exports.clubSquads = [
    ...
    // Warriors squads
    {
        ...
        memberCount: 16,
        primaryCoach: 'David Roberts',
        meetLocation: 'Southbank Fields',
        nextSession: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000).toISOString(),
        tags: ['Ball Mastery', 'Teamwork'],
        ageMin: 10,
        ageMax: 12,
    },
    {
        ...
        level: 'U14 · League',
        memberCount: 20,
        primaryCoach: 'Amy Taylor',
        meetLocation: 'Southbank Stadium',
        nextSession: new Date(today.getTime() + 2 * 24 * 60 * 60 * 1000).toISOString(),
        tags: ['Tactics', 'Match Play'],
        ageMin: 12,
        ageMax: 14,
    },
    {
        ...
        primaryCoach: 'Amy Taylor',
        meetLocation: 'Southbank Fields',
        nextSession: new Date(today.getTime() + 4 * 24 * 60 * 60 * 1000).toISOString(),
        tags: ['Technical', 'Empowerment'],
        ageMin: 13,
        ageMax: 16,
    },
    // Phoenix squads
    ...
    },
    {
        ...
        nextSession: new Date(today.getTime() + 5 * 24 * 60 * 60 * 1000).toISOString(),
        tags: ['Fun', 'Introduction'],
        ageMin: 5,
        ageMax: 8,
    },
];
...
function getAllClubMembershipsForUser(userId) {
    return exports.clubMemberships.filter((membership) => membership.userId === userId && membership.status === 'active');
}
function getClubById(clubId) {
    return exports.clubs.find((club) => club.id === clubId);
}
...
function getAggregatedFeed(userId, filter) {
    const memberships = getAllClubMembershipsForUser(userId);
    const clubIds = memberships.map((m) => m.clubId);
    let posts = exports.clubFeedPosts
...
function getClubSquads(clubId) {
    return exports.clubSquads.filter((squad) => squad.clubId === clubId);
}
function getClubFeed(clubId, filter) {
    let posts = exports.clubFeedPosts.filter((post) => post.clubId === clubId);
    if (filter && filter !== 'all') {
        posts = posts.filter((post) => post.postType === filter);
    }
...
function addClubFeedPost(post) {
    ...
    exports.clubFeedPosts.unshift(newPost);
    return newPost;
}
function togglePinPost(postId, pinnedBy) {
    const post = exports.clubFeedPosts.find((p) => p.id === postId);
    if (!post)
        return false;
...
function togglePostReaction(postId, userId) {
    const post = exports.clubFeedPosts.find((p) => p.id === postId);
    if (!post)
        return false;
    const key = `${postId}`;
...
function hasUserReacted(postId, userId) {
    const key = `${postId}`;
    return userReactions.get(key)?.has(userId) ?? false;
}
function getPinnedPosts(clubId) {
    return exports.clubFeedPosts.filter((post) => post.clubId === clubId && post.isPinned);
}
...
function getClubInvites(clubId) {
    return exports.clubInvites.filter((invite) => invite.clubId === clubId);
}
// ===== ADDITIONAL EXPORTS FOR COMPATIBILITY =====
// Chat threads for MessagesScreen
const BASE_CHAT_THREADS = exports.MOCK_CONVERSATIONS.map((conv) => ({
    id: conv.id,
    kind: 'direct',
...
// Chat threads for MessagesScreen
exports.chatThreads = [...BASE_CHAT_THREADS, ...GROUP_CHAT_THREADS];
// Chat messages for MessagesScreen
exports.chatMessages = [
    ...exports.MOCK_MESSAGES.map((msg) => ({
        id: msg.id,
...
exports.bookings = exports.MOCK_BOOKINGS;
// Mock athlete objectives
const MOCK_ATHLETE_OBJECTIVES = [
    {
        id: 'obj1',
        athleteId: 'user1',
        label: 'Finishing',
...
exports.activeObjectives = MOCK_ATHLETE_OBJECTIVES.filter(obj => obj.status === 'active');
exports.MOCK_DISCOVERY_COACHES = [
    {
        id: '1',
