...
export const DevSessionInfo = memo(function DevSessionInfo({
  athleteName, avatar, sessionDate, sessionBadges, colors, onAwardBadge,
}: DevSessionInfoProps) {
  return (
    <SurfaceCard style={styles.card}>
      <Row gap="sm" align="center">
        <View style={[styles.avatar, { backgroundColor: withAlpha(colors.tint, 0.12) }]}>
          <ThemedText style={[Typography.title, { color: colors.tint }]}>{avatar || athleteName.charAt(0)}</ThemedText>
        </View>
        <View style={{ flex: 1, gap: Spacing.micro }}>
          <ThemedText type="defaultSemiBold" style={Typography.subheading}>{athleteName}</ThemedText>
          <ThemedText style={[Typography.small, { color: colors.muted }]}>{sessionDate}</ThemedText>
        </View>
        <Clickable onPress={onAwardBadge} style={[styles.awardBtn, { backgroundColor: withAlpha(colors.tint, 0.09) }]}>
