...
export function RecurringCard({ recurring, onPause, onResume, onCancel, onPress, loading = false }: RecurringCardProps) {
  ...
  const handlePauseConfirm = useCallback(async () => {
    ...
    try { await onPause(recurring.id, pauseReason || undefined); setShowPauseModal(false); setPauseReason(''); }
  ...
  const handleCancelConfirm = useCallback(async () => {
    if (!onCancel) return;
    setActionLoading(true);
    try { await onCancel(recurring.id, cancelReason || undefined); setShowCancelModal(false); setCancelReason(''); }
    finally { setActionLoading(false); }
  ...
  const startDateLabel = new Date(recurring.startDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
  const freqLabel = getFrequencyLabel(recurring.frequency).toLowerCase();

  return (
    <>
      <SurfaceCard style={styles.card} onPress={handleCardPress} loading={loading || actionLoading}
        outlineGradient={recurring.status === 'ACTIVE' ? [statusColor, withAlpha(statusColor, 0.4)] : undefined}>
        ...
        <ActionsRow status={recurring.status} onPause={onPause ? () => setShowPauseModal(true) : undefined}
          onResume={onResume ? handleResume : undefined} onCancel={onCancel ? () => setShowCancelModal(true) : undefined} />
      </SurfaceCard>
