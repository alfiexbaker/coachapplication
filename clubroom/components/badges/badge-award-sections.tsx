...
interface AthleteHeaderProps {
  athleteName: string; athletePhotoUrl?: string; sessionLabel?: string; onClose: () => void;
}
...
interface BadgeSelectorProps {
  definitions: BadgeDefinition[]; selectedBadgeId: string | null; onSelect: (id: string) => void;
}
export const BadgeSelector = memo(function BadgeSelector({ definitions, selectedBadgeId, onSelect }: BadgeSelectorProps) {
  const { colors: palette } = useTheme();
  return (
    <View style={styles.section}>
      <ThemedText type="defaultSemiBold" style={styles.sectionTitle}>Choose Badge</ThemedText>
            ...
            <Animated.View key={badge.id} entering={FadeIn.delay(index * 50)}>
              ...
              >
                <View style={[styles.badgeCard, { backgroundColor: isSelected ? withAlpha(palette.tint, 0.09) : palette.surface, borderColor: isSelected ? palette.tint : palette.border }]}>
                  ...
                  <ThemedText style={[styles.badgeLabel, { color: isSelected ? palette.tint : palette.text }]} numberOfLines={2}>{badge.label}</ThemedText>
                  {isSelected && (
...
/* ─── Reason Selector ─── */
interface ReasonSelectorProps { selectedReason: string; onSelect: (reason: string) => void; }
export const ReasonSelector = memo(function ReasonSelector({ selectedReason, onSelect }: ReasonSelectorProps) {
  const { colors: palette } = useTheme();
  return (
    <View style={styles.section}>
...
/* ─── Note Input ─── */
interface NoteInputProps { note: string; onNoteChange: (v: string) => void; onQuickNote: (n: string) => void; }
export const NoteInput = memo(function NoteInput({ note, onNoteChange, onQuickNote }: NoteInputProps) {
  const { colors: palette } = useTheme();
  return (
    <View style={styles.section}>
      <ThemedText type="defaultSemiBold" style={styles.sectionTitle}>Add a note</ThemedText>
...
/* ─── Preview Section ─── */
interface PreviewSectionProps { selectedBadge: BadgeDefinition; selectedReason: string; note: string; coachName?: string; }
export const PreviewSection = memo(function PreviewSection({ selectedBadge, selectedReason, note, coachName }: PreviewSectionProps) {
  const { colors: palette } = useTheme();
  return (
...
/* ─── Error Banner ─── */
interface ErrorBannerProps { error: string; }
...
const styles = StyleSheet.create({
  ...
  badgeCard: { width: 100, padding: Spacing.md, borderRadius: Radii.lg, borderWidth: 1.5, alignItems: 'center', gap: Spacing.xs },
