...
export const CarpoolCreateModal = memo(function CarpoolCreateModal({
  visible, form, creating, onChangeForm, onSubmit, onClose,
        ...
        <KeyboardAvoidingView style={styles.body} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView contentContainerStyle={styles.formContent} showsVerticalScrollIndicator={false}>
            <View style={styles.field}>
              ...
              <TextInput style={inputStyle} placeholder="e.g., Saturday Training" placeholderTextColor={palette.muted} value={form.sessionName} onChangeText={(v) => onChangeForm({ ...form, sessionName: v })} />
            </View>
            <View style={styles.field}>
              <ThemedText type="defaultSemiBold" style={styles.label}>Date *</ThemedText>
              <TextInput style={inputStyle} placeholder="YYYY-MM-DD" placeholderTextColor={palette.muted} value={form.sessionDate} onChangeText={(v) => onChangeForm({ ...form, sessionDate: v })} />
            </View>
            <View style={styles.field}>
              <ThemedText type="defaultSemiBold" style={styles.label}>Available Seats *</ThemedText>
              <TextInput style={inputStyle} placeholder="2" placeholderTextColor={palette.muted} value={form.seatsAvailable} onChangeText={(v) => onChangeForm({ ...form, seatsAvailable: v })} keyboardType="number-pad" />
            </View>
            <View style={styles.field}>
              <ThemedText type="defaultSemiBold" style={styles.label}>Pickup Location *</ThemedText>
              <TextInput style={inputStyle} placeholder="e.g., High Street Car Park" placeholderTextColor={palette.muted} value={form.pickupLocation} onChangeText={(v) => onChangeForm({ ...form, pickupLocation: v })} />
            </View>
            <View style={styles.field}>
              <DateTimeField mode="time" label="Pickup Time *" value={form.pickupTime} onChange={(v) => onChangeForm({ ...form, pickupTime: v })} />
            </View>
            <View style={styles.field}>
              <Clickable
                style={[styles.toggleRow, { backgroundColor: form.returnOffered ? withAlpha(palette.tint, 0.09) : palette.surface, borderColor: form.returnOffered ? palette.tint : palette.border }]}
                onPress={() => onChangeForm({ ...form, returnOffered: !form.returnOffered })}
              >
                <Ionicons name={form.returnOffered ? 'checkbox' : 'square-outline'} size={22} color={form.returnOffered ? palette.tint : palette.muted} />
                <ThemedText style={styles.toggleLabel}>Offering return trip</ThemedText>
              </Clickable>
            </View>
            {form.returnOffered && (
              <View style={styles.field}>
                <ThemedText type="defaultSemiBold" style={styles.label}>Return Time</ThemedText>
                <TextInput style={inputStyle} placeholder="e.g., 12:00" placeholderTextColor={palette.muted} value={form.returnTime} onChangeText={(v) => onChangeForm({ ...form, returnTime: v })} />
              </View>
            )}
            <View style={styles.field}>
