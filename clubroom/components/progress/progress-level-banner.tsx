...
export const ProgressLevelBanner = memo(function ProgressLevelBanner({
  progress,
  colors,
}: ProgressLevelBannerProps) {
  return (
    <Row
      gap="md"
      align="center"
      style={[styles.banner, { backgroundColor: withAlpha(colors.tint, 0.06) }]}
    >
        ...
        <Row gap="xs" align="center">
          <View style={[styles.bar, { backgroundColor: withAlpha(colors.tint, 0.19) }]}>
            <View
              style={[
                styles.fill,
                { width: `${progress.progressToNextLevel}%`, backgroundColor: colors.tint },
              ]}
            />
          </View>
          <ThemedText style={[Typography.caption, { color: colors.muted, minWidth: 32 }]}>
            {progress.progressToNextLevel}%
          </ThemedText>
        </Row>
      </View>
      <View style={styles.points}>
        <ThemedText type="heading" style={[Typography.title, { color: colors.tint }]}>
          {progress.totalPoints}
        </ThemedText>
        <ThemedText style={[Typography.micro, { color: colors.muted, textTransform: 'uppercase' }]}>
          points
        </ThemedText>
      </View>
    </Row>
