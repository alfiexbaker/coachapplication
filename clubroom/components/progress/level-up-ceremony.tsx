...
export const LevelUpCeremony = forwardRef<LevelUpCeremonyRef, LevelUpCeremonyProps>(
  ({ onComplete }, ref) => {
    ...
    const hide = useCallback(() => {
      ...
      cardScale.value = withTiming(0.96, { duration: 200 });
      titleScale.value = withTiming(0.9, { duration: 180 });
      subtitleOpacity.value = withTiming(0, { duration: 150 });
      setTimeout(() => {
        setVisible(false);
        onComplete?.();
      }, 250);
    }, [cardScale, clearTimer, onComplete, overlayOpacity, subtitleOpacity, titleScale]);

    const show = useCallback(
      (payload: { levelName: string; tier: CardTier }) => {
        clearTimer();
        ...
        cardScale.value = withSpring(1, { damping: 12, stiffness: 105 });
        titleScale.value = withDelay(260, withSpring(1, { damping: 11, stiffness: 110 }));
      ...
      [cardScale, clearTimer, hide, overlayOpacity, subtitleOpacity, titleScale],
    );
    ...
    return (
      <Modal transparent visible={visible} animationType="none">
        <Animated.View
          style={[
            styles.overlay,
            { backgroundColor: withAlpha(colors.text, 0.74) },
            overlayStyle,
          ]}
        >
          <Animated.View
          ...
          >
            <Column align="center" gap="sm">
              ...
              <Animated.View style={titleStyle}>
                <ThemedText style={[styles.title, { color: colors.onPrimary }]}>
                  LEVEL UP
                </ThemedText>
              </Animated.View>
