...
export const FeedTypeSelector = memo(function FeedTypeSelector({ feedType, clubName, onSelect }: FeedTypeSelectorProps) {
  const { colors: palette } = useTheme();
  const opts: { key: FeedType; label: string; icon: string; color: string }[] = [
    { key: 'PERSONAL', label: 'My Personal Feed', icon: 'person-circle-outline', color: palette.success },
    ...(clubName ? [
...
interface PostTypeSelectorProps { postType: ClubPostType; onSelect: (pt: ClubPostType) => void; }

export const PostTypeSelector = memo(function PostTypeSelector({ postType, onSelect }: PostTypeSelectorProps) {
  const { colors: palette } = useTheme();
  return (
    <View style={styles.section}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.row}>
        {POST_TYPES.map((type) => (
          <Clickable key={type.key} style={[styles.chip, { borderColor: postType === type.key ? palette.tint : palette.border }, postType === type.key ? { backgroundColor: withAlpha(palette.tint, 0.06) } : undefined]} onPress={() => onSelect(type.key)}>
            <Ionicons name={type.icon as keyof typeof Ionicons.glyphMap} size={20} color={postType === type.key ? palette.tint : palette.muted} />
            <ThemedText style={[styles.chipLabel, { color: postType === type.key ? palette.tint : palette.text }]}>{type.label}</ThemedText>
          </Clickable>
        ))}
      </ScrollView>
    </View>
...
interface PostAsSelectorProps { postAs: 'self' | 'club'; onSelect: (pa: 'self' | 'club') => void; }

export const PostAsSelector = memo(function PostAsSelector({ postAs, onSelect }: PostAsSelectorProps) {
  const { colors: palette } = useTheme();
  return (
    <View style={styles.section}>
...
export const AudienceSelector = memo(function AudienceSelector({ audienceType, selectedSquadId, squads, onSelectClub, onSelectSquad, onSelectSquadId }: AudienceSelectorProps) {
  const { colors: palette } = useTheme();
  ...
  return (
    <View style={styles.section}>
      ...
      <Row gap="sm">
        ...
        <Clickable style={[styles.twoColOption, { borderColor: audienceType === 'squad' ? palette.tint : palette.border }, audienceType === 'squad' ? { backgroundColor: withAlpha(palette.tint, 0.06) } : undefined]} onPress={onSelectSquad}>
      ...
      {audienceType === 'squad' && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.row, { marginTop: Spacing.sm }]}>
          {squads.map((squad) => (
            <Clickable key={squad.id} style={[styles.squadOption, { borderColor: selectedSquadId === squad.id ? palette.success : palette.border, backgroundColor: selectedSquadId === squad.id ? withAlpha(palette.success, 0.06) : palette.surface }]} onPress={() => onSelectSquadId(squad.id)}>
              <View style={[styles.squadBadge, { backgroundColor: selectedSquadId === squad.id ? palette.success : palette.muted }]}>
                <ThemedText style={{ color: palette.onPrimary, ...Typography.micro }}>{squad.name.slice(0, 2).toUpperCase()}</ThemedText>
              </View>
              <View style={{ flex: 1 }}>
              ...
              {selectedSquadId === squad.id && <Ionicons name="checkmark-circle" size={18} color={palette.success} />}
            </Clickable>
          ))}
        </ScrollView>
      )}
