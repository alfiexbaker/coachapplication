...
export function WeekPatternGrid(props: WeekPatternGridProps) {
  const { templates, overrides, blockedDates, coachId, isSetupMode, onDayPress, onSetupComplete, onTimeOffPress } = props;
  ...
  const weekLabel = useMemo(() => {
    ...
    return sM === eM ? `${viewedWeekStart.getDate()} - ${end.getDate()} ${sM}` : `${viewedWeekStart.getDate()} ${sM} - ${end.getDate()} ${eM}`;
  }, [weekOffset, viewedWeekStart]);

  // Summary stats
  const stats = useMemo(() => {
    let mins = 0; const days = new Set<number>(); const venues = new Set<string>();
    for (const d of DAYS_ORDERED) {
      const dateStr = getDateForDay(d.index);
      if (blockedDates.has(dateStr) || overrides.some(o => o.date === dateStr && o.isBlocked)) continue;
      const ovr = overrides.find(o => o.date === dateStr && !o.isBlocked && (o.customSlots?.length ?? 0) > 0);
      const slots = ovr?.customSlots ?? (weekOffset === 0 ? templates.filter(t => t.dayOfWeek === d.index) : templates.filter(t => t.dayOfWeek === d.index));
      for (const s of slots) { mins += timeToMinutes(s.endTime) - timeToMinutes(s.startTime); days.add(d.index); if (s.location) venues.add(s.location); }
    }
  ...
  // Helpers
  const getTemplatesForDay = useCallback((dow: number) => templates.filter(t => t.dayOfWeek === dow), [templates]);

  const getOverrideForDate = useCallback((dateStr: string) => {
    const isBlocked = blockedDates.has(dateStr) || overrides.some(o => o.date === dateStr && o.isBlocked);
    const ovr = overrides.find(o => o.date === dateStr && !o.isBlocked && (o.customSlots?.length ?? 0) > 0) ?? null;
  ...
  const makeBlockedOverride = (dateStr: string): AvailabilityOverride =>
    overrides.find(o => o.date === dateStr && o.isBlocked) ?? { id: `legacy_${dateStr}`, coachId, date: dateStr, isBlocked: true, reason: 'Time Off' };
  ...
  const renderDayRows = (d: typeof DAYS_ORDERED[number], idx: number) => {
    ...
    if (isBlocked) {
      ...
      return (
        <View key={d.index}>
          <WeekPatternSlotRow dayLabel={d.short} showDayLabel timeDisplay={eff.reason || 'Time Off'} locationDisplay={null} hasOverride={false} isBlocked blockReason={eff.reason} isToday={isToday} hasTemplates={false} showSeparator={!isLast} onPress={() => onTimeOffPress?.(dateStr, eff)} />
        </View>
      );
    }
    ...
    if (hasOverride && override?.customSlots) {
      const slots = [...override.customSlots].sort((a, b) => a.startTime.localeCompare(b.startTime));
      return (
        <View key={d.index}>
          {slots.map((slot, si) => (
            <WeekPatternSlotRow key={`${d.index}-o-${si}`} dayLabel={d.short} showDayLabel={si === 0} timeDisplay={formatTimeRange(slot.startTime, slot.endTime)} locationDisplay={slot.location ?? null} hasOverride isBlocked={false} isToday={false} hasTemplates showSeparator={false} onPress={() => onDayPress(d.index, undefined, dateStr)} />
          ))}
          <WeekPatternAddBlockRow dayIndex={d.index} showSeparator={!isLast} onPress={() => onDayPress(d.index, undefined, dateStr)} />
        </View>
      );
    }
    ...
    if (sorted.length === 0) {
      return (
        <View key={d.index}>
          <WeekPatternSlotRow dayLabel={d.short} showDayLabel timeDisplay="--" locationDisplay={null} hasOverride={weekOffset === 0 && hasOverride} isBlocked={false} isToday={isToday} hasTemplates={false} showSeparator={!isLast} onPress={() => onDayPress(d.index, undefined, dateStr)} />
        </View>
      );
    }
    ...
    return (
      <View key={d.index}>
        {sorted.map((tmpl, ti) => (
          <WeekPatternSlotRow key={tmpl.id} dayLabel={d.short} showDayLabel={ti === 0} timeDisplay={formatTimeRange(tmpl.startTime, tmpl.endTime)} locationDisplay={tmpl.location ?? null} hasOverride={ti === 0 && (weekOffset === 0 ? hasOverride : false)} isBlocked={false} isToday={isToday} hasTemplates showSeparator={false} onPress={() => onDayPress(d.index, tmpl.id, dateStr)} />
        ))}
        <WeekPatternAddBlockRow dayIndex={d.index} showSeparator={!isLast} onPress={() => onDayPress(d.index, undefined, dateStr)} />
      </View>
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.weekNavRow}>
        ...
        <Clickable onPress={() => { if (weekOffset !== 0) { haptic(); setWeekOffset(0); } }} style={styles.weekNavLabelBtn}>
          <ThemedText style={[styles.weekNavLabel, { color: palette.text }]}>{weekLabel}</ThemedText>
        </Clickable>
        <Clickable onPress={() => { haptic(); setWeekOffset(p => p + 1); }} style={styles.weekNavArrow}>
...
const styles = StyleSheet.create({
  ...
  summaryRow: { paddingVertical: Spacing.sm, paddingHorizontal: Spacing.md, borderRadius: Radii.md },
