  ...
  return (
    <View style={[styles.tierTable, Shadows[scheme].card, { backgroundColor: palette.surface }]}>
      <Row style={[styles.tierHeaderRow, { backgroundColor: palette.background }]}>
        <ThemedText style={[styles.tierHeaderCell, { flex: 2, color: palette.muted }]}>
          Cancellation window
        </ThemedText>
        <ThemedText
          style={[styles.tierHeaderCell, { flex: 1, textAlign: 'right', color: palette.muted }]}
        >
          Refund
        </ThemedText>
      </Row>
      {sorted.map((tier, idx) => {
        ...
        return (
          ...
          >
            <ThemedText
              style={[styles.tierCell, { flex: 2, color: palette.text }]}
              numberOfLines={2}
            >
              {tier.description}
            </ThemedText>
...
export const EditableTierRow = memo(function EditableTierRow({
  tier,
  index,
  onUpdate,
  onRemove,
  ...
  return (
    <View
      style={[styles.editableTierRow, Shadows[scheme].subtle, { backgroundColor: palette.surface }]}
    >
      <Row style={styles.editableTierFields}>
        <View style={styles.editableField}>
          <ThemedText style={[styles.editableFieldLabel, { color: palette.muted }]}>
            Hours before
          </ThemedText>
          <TextInput
            ...
            onChangeText={(text) => {
              ...
              if (!isNaN(n) && n >= 0) onUpdate(index, 'hoursBeforeSession', n);
            }}
          />
        </View>
        <View style={styles.editableField}>
          ...
          <TextInput
            ...
            onChangeText={(text) => {
              const n = parseInt(text, 10);
              if (!isNaN(n) && n >= 0 && n <= 100) onUpdate(index, 'refundPercentage', n);
            }}
          />
        </View>
      </Row>
