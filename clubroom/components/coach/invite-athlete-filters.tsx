...
function FilterPanelInner(p: FilterPanelProps) {
          ...
          {options.map((opt) => (
            <Clickable
              ...
              style={[
                styles.filterChip,
                {
                  backgroundColor: current === opt ? palette.tint : 'transparent',
                  borderColor: current === opt ? palette.tint : palette.border,
                },
              ]}
            >
              <ThemedText
                style={{
                  ...Typography.caption,
                  color: current === opt ? palette.onPrimary : palette.text,
                }}
              >
                {format
                  ? format(opt)
  ...
  return (
    <View style={[styles.panel, { backgroundColor: palette.surface, borderColor: palette.border }]}>
      {renderChipRow(
        'Skill Level',
        ['ALL', 'BEGINNER', 'INTERMEDIATE', 'ADVANCED'] as SkillFilter[],
        p.skillFilter,
        p.onSkillFilter,
      )}
      {renderChipRow(
        'Age Group',
        ['ALL', 'U8', 'U10', 'U12', 'U14', 'U16', '16+'] as AgeFilter[],
        p.ageFilter,
        p.onAgeFilter,
        (v) => (v === 'ALL' ? 'All' : v),
      )}
      {p.availableSquads.length > 0 &&
        renderChipRow(
          'Squad',
    ...
    <View style={styles.quickContainer}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <Row style={styles.quickActions}>
          <Clickable
            onPress={p.onSelectAll}
            style={[styles.quickButton, { backgroundColor: withAlpha(palette.tint, 0.06) }]}
          >
            <Ionicons name="checkmark-done" size={14} color={palette.tint} />
            <ThemedText style={{ color: palette.tint, ...Typography.caption }}>
              Select All ({p.filteredCount})
            </ThemedText>
          </Clickable>
          <Clickable
            onPress={p.onSelectNone}
            style={[
              styles.quickButton,
          ...
          {p.availableSquads.map((squad) => (
            <Clickable
              key={squad.id}
              onPress={() => p.onSelectBySquad(squad.id)}
              style={[
                styles.quickButton,
                { backgroundColor: palette.surface, borderColor: palette.border, borderWidth: 1 },
              ]}
          ...
          <Clickable
            onPress={() => p.onSelectBySkill('BEGINNER')}
            style={[
              styles.quickButton,
              { backgroundColor: palette.surface, borderColor: palette.border, borderWidth: 1 },
            ]}
