          ...
          {athletes.map((entry) => (
            <Clickable key={entry.athlete.id}
              ...
              <View style={styles.actionRow}>
                {entry.needsNotes && <View style={[styles.pill, { backgroundColor: withAlpha(palette.error, 0.06) }]}><Ionicons name="document-text" size={12} color={palette.error} /><ThemedText style={[styles.pillLabel, { color: palette.error }]}>Add notes</ThemedText></View>}
                {entry.averageRating < 4 && <View style={[styles.pill, { backgroundColor: withAlpha(palette.tint, 0.06) }]}><Ionicons name="trending-up" size={12} color={palette.tint} /><ThemedText style={[styles.pillLabel, { color: palette.tint }]}>Boost rating</ThemedText></View>}
                {entry.daysSinceLast >= 10 && <View style={[styles.pill, { backgroundColor: withAlpha(palette.icon, 0.06) }]}><Ionicons name="time" size={12} color={palette.icon} /><ThemedText style={[styles.pillLabel, { color: palette.icon }]}>Reach out</ThemedText></View>}
              </View>
            </Clickable>
          ))}
        </View>
    ...
    <SurfaceCard style={styles.sectionCard}>
      ...
      <View style={{ gap: Spacing.xs }}>
        {sessions.map((session) => {
          ...
          return (
            <View key={session.id} style={[styles.recentRow, { borderColor: palette.border }]}>
              <View style={styles.rowLeft}>
                <View style={[styles.avatar, { backgroundColor: withAlpha(palette.tint, 0.12) }]}>
...
const styles = StyleSheet.create({
  ...
  completionHeader: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm },
  completionIcon: { width: 36, height: 36, borderRadius: Radii.xl, alignItems: 'center', justifyContent: 'center' },
  completionRow: { flexDirection: 'row', alignItems: 'center', padding: Spacing.sm, borderRadius: Radii.sm, borderWidth: 1, gap: Spacing.sm },
  ...
  actionRow: { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.xxs },
  pill: { flexDirection: 'row', alignItems: 'center', gap: Spacing.xxs, paddingHorizontal: Spacing.xs, paddingVertical: Spacing.xxs, borderRadius: Radii.pill },
  pillLabel: { ...Typography.caption, letterSpacing: -0.1 },
  actionPill: { flexDirection: 'row', alignItems: 'center', gap: Spacing.xs, paddingHorizontal: Spacing.md, paddingVertical: Spacing.xs, borderRadius: Radii.md, borderWidth: 1 },
  ...
  badgesShortcut: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm, padding: Spacing.md, borderRadius: Radii.card, borderWidth: 1 },
