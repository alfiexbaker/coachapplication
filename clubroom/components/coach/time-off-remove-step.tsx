...
function TimeOffRemoveStepInner({ removeDate, existingOverride, removing, onConfirmRemove, onBack }: TimeOffRemoveStepProps) {
  ...
  return (
    <View style={styles.content}>
      ...
      <Clickable onPress={onConfirmRemove} disabled={removing} style={[styles.primaryBtn, { backgroundColor: palette.error }]} accessibilityLabel="Confirm remove time off">
        {removing ? (
          <ThemedText style={[styles.primaryBtnText, { color: palette.onPrimary }]}>Removing...</ThemedText>
        ) : (
          <>
            <Ionicons name="trash-outline" size={18} color={palette.onPrimary} />
            <ThemedText style={[styles.primaryBtnText, { color: palette.onPrimary }]}>Yes, Remove Time Off</ThemedText>
          </>
        )}
      </Clickable>

      <Clickable onPress={onBack} disabled={removing} style={styles.backBtn} accessibilityLabel="Cancel removal">
        <Ionicons name="arrow-back" size={16} color={palette.muted} />
        <ThemedText style={[styles.backBtnText, { color: palette.muted }]}>Keep Time Off</ThemedText>
      </Clickable>
    </View>
  );
}
