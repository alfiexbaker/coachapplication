...
export function InviteAthleteModal({ visible, onClose, onSelect, athletes, squads = [], multiSelect = true, title = 'Select Athletes' }: InviteAthleteModalProps) {
  ...
  const handleToggle = useCallback((athlete: Athlete) => {
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={handleClose}>
      <View style={[styles.container, { backgroundColor: palette.background }]}>
        ...
        <View style={styles.searchContainer}>
          <Row style={[styles.searchBar, { backgroundColor: palette.surface, borderColor: palette.border }]}>
            ...
            {state.searchQuery.length > 0 && (
              <Clickable accessibilityLabel="Clear search" onPress={() => state.setSearchQuery('')}><Ionicons name="close-circle" size={18} color={palette.muted} /></Clickable>
            )}
        ...
        {state.showFilters && (
          <FilterPanel
            skillFilter={state.skillFilter} ageFilter={state.ageFilter} squadFilter={state.squadFilter}
            availableSquads={state.availableSquads} hasActiveFilters={state.hasActiveFilters}
            onSkillFilter={state.setSkillFilter} onAgeFilter={state.setAgeFilter} onSquadFilter={state.setSquadFilter} onReset={state.resetFilters}
          />
        )}

        {/* Quick Select */}
        {multiSelect && (
          <QuickSelect filteredCount={state.filteredAthletes.length} availableSquads={state.availableSquads}
            onSelectAll={state.selectAll} onSelectNone={state.selectNone} onSelectBySquad={state.selectBySquad} onSelectBySkill={state.selectBySkillLevel} />
        )}
        ...
        {/* List */}
        <AthleteList groupedByParent={state.groupedByParent} selectedAthletes={state.selectedAthletes}
          searchQuery={state.searchQuery} filteredCount={state.filteredAthletes.length} onToggle={handleToggle} />
      </View>
    </Modal>
  );
}
