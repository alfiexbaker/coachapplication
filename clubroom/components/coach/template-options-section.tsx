...
function TemplateOptionsSectionInner({
  ...
  maxConcurrent,
  onSelectMaxConcurrent,
  bufferMinutes,
  onSelectBufferMinutes,
  isEditing,
  isQuickAdd,
  saving,
  canDelete,
    ...
    <>
      ...
      <View style={styles.section}>
        ...
        <Row style={styles.typeRow}>
          <Clickable
            ...
            style={[
              ...
              {
                backgroundColor:
                  maxConcurrent === 1 ? withAlpha(palette.tint, 0.07) : palette.surface,
          ...
          >
            <View
              style={[
                ...
                {
                  backgroundColor:
                    maxConcurrent === 1 ? withAlpha(palette.tint, 0.12) : palette.background,
                },
              ]}
            >
              <Ionicons
                name="person-outline"
                size={22}
                color={maxConcurrent === 1 ? palette.tint : palette.muted}
              />
            </View>
            <ThemedText
              style={[
                styles.typeLabel,
                { color: maxConcurrent === 1 ? palette.tint : palette.text },
              ]}
            >
              1v1 Session
            </ThemedText>
          ...
          <Clickable
            onPress={() => {
              if (maxConcurrent <= 1) onSelectMaxConcurrent(4);
            }}
            style={[
              styles.typeOption,
              {
                backgroundColor:
                  maxConcurrent > 1 ? withAlpha(palette.info, 0.07) : palette.surface,
          ...
          >
            <View
              style={[
                ...
                {
                  backgroundColor:
                    maxConcurrent > 1 ? withAlpha(palette.info, 0.12) : palette.background,
                },
              ]}
            >
              <Ionicons
                name="people-outline"
                size={22}
                color={maxConcurrent > 1 ? palette.info : palette.muted}
              />
            </View>
            <ThemedText
              style={[styles.typeLabel, { color: maxConcurrent > 1 ? palette.info : palette.text }]}
            >
              Group Session
            </ThemedText>
            <ThemedText style={[styles.typeDesc, { color: palette.muted }]}>
        ...
        {maxConcurrent > 1 && (
          <Row
            style={[
              styles.groupSizeRow,
              { backgroundColor: withAlpha(palette.info, 0.03), borderColor: palette.border },
            ]}
          >
            ...
            <Row style={styles.stepper}>
              <Clickable
                accessibilityLabel="Decrease max group size"
                onPress={() => onSelectMaxConcurrent(Math.max(2, maxConcurrent - 1))}
                style={[
              ...
              <Clickable
                accessibilityLabel="Increase max group size"
                onPress={() => onSelectMaxConcurrent(maxConcurrent + 1)}
                style={[
                  styles.stepperButton,
                  { borderColor: palette.border, backgroundColor: palette.surface },
                ]}
      ...
      {isEditing && canDelete && (
        ...
        >
          ...
          <ThemedText style={{ ...Typography.bodySemiBold, color: palette.error }}>
            Delete This Slot
          </ThemedText>
        </Clickable>
      )}
