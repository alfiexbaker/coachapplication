...
interface ProfileTabAboutProps {
  coach: {
    bio?: string; shortBio?: string; email?: string; phone?: string; website?: string;
    socialLinks?: SocialLinksType; experiences?: CoachExperience[];
    certifications?: CoachCertification[]; achievements?: string[];
    languages?: CoachLanguage[]; footballFocuses: string[];
  };
  userRole?: string;
}
  ...
  return (
    <View style={styles.aboutContent}>
      ...
      <SurfaceCard style={styles.section}>
        ...
        {coach.email && (
          <Clickable style={styles.contactItem} onPress={() => Linking.openURL(`mailto:${coach.email}`)}>
            <Ionicons name="mail" size={20} color={palette.tint} />
            <ThemedText style={styles.contactText} numberOfLines={1}>{coach.email}</ThemedText>
          </Clickable>
        )}
        {coach.phone && (
          <Clickable style={styles.contactItem} onPress={() => Linking.openURL(`tel:${coach.phone}`)}>
        ...
        {coach.experiences && coach.experiences.length > 0 ? coach.experiences.map((exp) => (
          <Row key={exp.id} style={styles.credentialItem}>
            ...
            <View style={styles.credentialContent}>
              ...
              <ThemedText style={styles.credentialDate}>{formatDate(exp.startDate)} - {exp.current ? 'Present' : formatDate(exp.endDate!)}</ThemedText>
          ...
          return (
            <Row key={cert.id} style={styles.credentialItem}>
              ...
              <View style={styles.credentialContent}>
                ...
                <ThemedText style={styles.credentialDate}>Issued {formatFullDate(cert.issueDate)}{cert.expiryDate && ` \u2022 Expires ${formatFullDate(cert.expiryDate)}`}</ThemedText>
...
const styles = StyleSheet.create({
  ...
  specialtyTag: { paddingHorizontal: Spacing.md, paddingVertical: Spacing.sm, borderRadius: Radii.md, borderWidth: 1 },
