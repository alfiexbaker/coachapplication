    ...
    <>
      ...
      <View style={styles.coverContainer}>
        ...
        <View style={styles.headerButtons}>
          <Clickable onPress={() => router.back()} style={[styles.headerBtn, { backgroundColor: withAlpha(palette.text, 0.4) }]}>
            <Ionicons name="arrow-back" size={Components.icon.lg} color={palette.onPrimary} />
          </Clickable>
          <Clickable accessibilityLabel="Share profile" onPress={onShare} style={[styles.headerBtn, { backgroundColor: withAlpha(palette.text, 0.4) }]}>
      ...
      <View style={styles.profileInfo}>
        <View style={styles.nameRow}>
          <ThemedText style={[Typography.display, { color: palette.text, flex: 1 }]}>{coach.name}</ThemedText>
        ...
        <View style={styles.ratingRow}>
          <View style={styles.starsRow}>
            {stars.map((s, i) => <Ionicons key={i} name={s.name} size={14} color={s.color} />)}
          </View>
          <ThemedText style={[Typography.bodySemiBold, { color: palette.text }]}>{coach.rating.toFixed(1)}</ThemedText>
          <ThemedText style={[Typography.small, { color: palette.muted }]}>({coach.reviewCount} reviews)</ThemedText>
        </View>
        {coach.location ? (
        ...
        {coach.badges && coach.badges.length > 0 ? (
          <View style={styles.badgesRow}>
            {coach.badges.map((badge, i) => (
              <View key={i} style={[styles.badgePill, { backgroundColor: withAlpha(palette.success, 0.09) }]}>
                <Ionicons name="checkmark-circle" size={12} color={palette.success} />
                <ThemedText style={[Typography.caption, { color: palette.success }]}>{badge}</ThemedText>
              </View>
            ))}
          </View>
        ) : null}
        <View style={styles.ctaRow}>
          <Clickable onPress={onBook} style={[styles.bookButton, { backgroundColor: palette.tint }]}>
            <Ionicons name="calendar-outline" size={Components.icon.md} color={palette.surface} />
            <ThemedText style={[Typography.bodySemiBold, { color: palette.surface }]}>Book a Session</ThemedText>
          </Clickable>
...
const styles = StyleSheet.create({
  ...
  coverPlaceholder: { width: SCREEN_WIDTH, height: COVER_HEIGHT, opacity: 0.85, alignItems: 'center', justifyContent: 'center' },
  headerButtons: { position: 'absolute', top: Spacing.sm, left: Spacing.sm, right: Spacing.sm, flexDirection: 'row', justifyContent: 'space-between' },
  headerBtn: { width: 40, height: 40, borderRadius: Radii.xl, alignItems: 'center', justifyContent: 'center' },
  ...
  avatarPlaceholder: { width: AVATAR_SIZE, height: AVATAR_SIZE, borderRadius: AVATAR_SIZE / 2, borderWidth: 4, alignItems: 'center', justifyContent: 'center' },
  ...
  verifiedBadge: { flexDirection: 'row', alignItems: 'center', gap: Spacing.xs / 2, paddingHorizontal: Spacing.xs, paddingVertical: Spacing.xs / 2, borderRadius: Radii.pill },
  ratingRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.xs },
  starsRow: { flexDirection: 'row', gap: Spacing.micro },
  locationRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.xs / 2 },
  badgesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.xs, marginTop: Spacing.xs / 2 },
  badgePill: { flexDirection: 'row', alignItems: 'center', gap: Spacing.xs / 2, paddingHorizontal: Spacing.xs, paddingVertical: Spacing.xs / 2, borderRadius: Radii.pill },
  ctaRow: { flexDirection: 'row', gap: Spacing.sm, marginTop: Spacing.sm },
  bookButton: { flex: 1, height: Components.button.height, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: Spacing.xs, borderRadius: Components.button.borderRadius },
  messageButton: { width: Components.button.height, height: Components.button.height, borderRadius: Radii.md, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center' },
});
