...
import type { CoachQueueLaneKey } from '@/utils/progress-task-time';

interface LaneOption {
  id: CoachQueueLaneKey;
  label: string;
  count: number;
}

interface CoachLaneSegmentProps {
  options: LaneOption[];
  selectedId: CoachQueueLaneKey;
  onSelect: (value: CoachQueueLaneKey) => void;
  reduceMotion?: boolean;
}

export const CoachLaneSegment = memo(function CoachLaneSegment({
  options,
  selectedId,
  onSelect,
  reduceMotion = false,
    ...
    >
      ...
      <Row style={styles.row}>
        {options.map((option) => {
          ...
          const countColor = option.id === 'intervene_now' && option.count > 0 ? colors.error : selected ? colors.tint : colors.muted;
          ...
          return (
            <Clickable
              ...
              accessibilityLabel={`Show ${option.label.toLowerCase()} lane. ${option.count} athletes.`}
            >
              <ThemedText style={[styles.label, { color: selected ? colors.tint : colors.text }]} numberOfLines={1}>
                {option.label}
              </ThemedText>
              <View
                style={[
