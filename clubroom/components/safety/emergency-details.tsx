    ...
    <>
        ...
        <Animated.View entering={FadeInDown.delay(50).springify()}>
          <SurfaceCard style={styles.section}>
            <Row align="center" gap="sm">
              <View
                style={[styles.sectionIcon, { backgroundColor: withAlpha(palette.error, 0.06) }]}
              >
      ...
      <Animated.View entering={FadeInDown.delay(100).springify()}>
        <SurfaceCard style={styles.section}>
          <Row align="center" gap="sm">
            <View
              style={[styles.sectionIcon, { backgroundColor: withAlpha(palette.success, 0.06) }]}
            >
            ...
            <View style={styles.contactsList}>
              {data.allContacts.map((contact) => (
                <EmergencyContactCard
                  key={contact.id}
                  contact={contact}
                  onCall={() => onCallContact(contact.phone, contact.name)}
                />
              ))}
            </View>
          ) : (
      ...
      {(data.doctorName || data.doctorPhone) && (
        <Animated.View entering={FadeInDown.delay(150).springify()}>
          <SurfaceCard style={styles.section}>
            <Row align="center" gap="sm">
              <View
                style={[styles.sectionIcon, { backgroundColor: withAlpha(palette.tint, 0.06) }]}
              >
      ...
      <Animated.View entering={FadeInDown.delay(200).springify()}>
        <SurfaceCard style={styles.section}>
          ...
          <Row align="center" justify="between">
              ...
              style={[
                styles.consentBadge,
                {
                  backgroundColor: withAlpha(
                    data.emergencyTreatmentConsent ? palette.success : palette.error,
                    0.06,
                  ),
                },
              ]}
            >
              <Ionicons
                name={data.emergencyTreatmentConsent ? 'checkmark-circle' : 'close-circle'}
                size={16}
                color={data.emergencyTreatmentConsent ? palette.success : palette.error}
              />
              <ThemedText
                style={[
                  styles.consentText,
      ...
      <Animated.View entering={FadeInDown.delay(250).springify()}>
        <SafetyChecklist
          hasEmergencyContact={data.allContacts.length > 0}
          hasEmergencyConsent={data.emergencyTreatmentConsent}
          hasMedicalInfo={data.hasAlerts}
        />
      </Animated.View>
