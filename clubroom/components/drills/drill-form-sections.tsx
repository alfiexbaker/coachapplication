  ...
  return (
    <Row style={styles.optionsRow}>
      {CATEGORIES.map((cat) => {
        ...
        return (
          <Clickable key={cat} onPress={() => onSelect(cat)}
            style={[styles.categoryOption, { backgroundColor: isSelected ? withAlpha(info.color, 0.12) : palette.surface, borderColor: isSelected ? info.color : palette.border }]}>
            <Ionicons name={info.icon as keyof typeof Ionicons.glyphMap} size={16} color={isSelected ? info.color : palette.muted} />
            <ThemedText style={[styles.categoryOptionText, { color: isSelected ? info.color : palette.text }]}>{info.label}</ThemedText>
          </Clickable>
        );
      })}
    </Row>
...
/* ─── Difficulty Picker ─── */
interface DifficultyPickerProps { selected: DrillDifficulty; onSelect: (d: DrillDifficulty) => void; }
  ...
  return (
    <Row style={styles.difficultyRow}>
      {DIFFICULTIES.map((diff) => {
        ...
        return (
          <Clickable key={diff} onPress={() => onSelect(diff)}
            style={[styles.difficultyOption, { backgroundColor: isSelected ? info.bgColor : palette.surface, borderColor: isSelected ? info.color : palette.border }]}>
            <ThemedText style={[styles.difficultyOptionText, { color: isSelected ? info.color : palette.text }]}>{info.label}</ThemedText>
          </Clickable>
        );
      })}
    </Row>
...
interface DurationPickerProps { duration: string; onDurationChange: (v: string) => void; error?: string; }
export const DurationPicker = memo(function DurationPicker({ duration, onDurationChange, error }: DurationPickerProps) {
  const { colors: palette } = useTheme();
  return (
    <View>
      <Row style={styles.durationRow}>
        {DURATION_PRESETS.map((mins) => (
          <Clickable key={mins} onPress={() => onDurationChange(mins.toString())}
            ...
            <ThemedText style={[styles.durationOptionText, { color: duration === mins.toString() ? palette.onPrimary : palette.text }]}>{mins}</ThemedText>
          </Clickable>
        ))}
        <TextInput
          style={[styles.durationInput, { backgroundColor: palette.surface, borderColor: error ? palette.error : palette.border, color: palette.text }]}
...
/* ─── Video URL Input ─── */
interface VideoUrlInputProps { videoUrl: string; onVideoUrlChange: (v: string) => void; error?: string; }
export const VideoUrlInput = memo(function VideoUrlInput({ videoUrl, onVideoUrlChange, error }: VideoUrlInputProps) {
  const { colors: palette } = useTheme();
  return (
    <View>
      <View style={styles.inputWithIcon}>
        ...
        <TextInput
          style={[styles.input, styles.inputWithIconInput, { backgroundColor: palette.surface, borderColor: error ? palette.error : palette.border, color: palette.text }]}
          placeholder="https://..." placeholderTextColor={palette.muted}
          value={videoUrl} onChangeText={onVideoUrlChange}
