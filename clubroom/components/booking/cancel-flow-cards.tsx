...
interface RefundBannerProps { calculation: RefundCalculation; colors: Record<string, string>; }
...
interface ReasonCardProps { option: ReasonOption; selected: boolean; onPress: () => void; colors: Record<string, string>; }

function ReasonCardInner({ option, selected, onPress, colors }: ReasonCardProps) {
  return (
    <Clickable style={[styles.reasonCard, { backgroundColor: colors.surface, borderColor: colors.border }, selected ? { borderColor: colors.tint, backgroundColor: withAlpha(colors.tint, 0.03) } : undefined]} onPress={onPress} accessibilityRole="radio" accessibilityState={{ selected }}>
      <Ionicons name={option.icon} size={18} color={selected ? colors.tint : colors.muted} />
      <ThemedText style={[styles.reasonLabel, { color: colors.text }, selected ? { ...Typography.bodySemiBold, color: colors.tint } : undefined]} numberOfLines={1}>{option.label}</ThemedText>
