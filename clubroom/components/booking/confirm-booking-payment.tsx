...
export const ConfirmBookingPayment = memo(function ConfirmBookingPayment({
  cardNumber, expiryDate, cvv, isProcessing, onCardChange, onExpiryChange, onCvvChange,
}: ConfirmBookingPaymentProps) {
  const { colors: palette } = useTheme();
  const containerStyle = [styles.inputContainer, { backgroundColor: palette.background, borderColor: palette.border }];

  return (
    <View style={styles.section}>
      <ThemedText type="defaultSemiBold" style={styles.sectionTitle}>Payment Details</ThemedText>
      <SurfaceCard style={styles.card}>
        ...
        <View style={styles.inputRow}>
          <View style={[styles.inputGroup, { flex: 1 }]}>
            ...
            <View style={containerStyle}>
              <TextInput value={expiryDate} onChangeText={onExpiryChange} placeholder="MM/YY" placeholderTextColor={palette.muted} keyboardType="number-pad" editable={!isProcessing} style={[styles.input, { color: palette.text }]} />
            </View>
          </View>
          <View style={[styles.inputGroup, { flex: 1 }]}>
