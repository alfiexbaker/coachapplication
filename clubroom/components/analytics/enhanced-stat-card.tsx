...
export function EnhancedStatCard({ value, label, icon, iconColor, trend, comparison, sparklineData, variant = 'default', delay = 0 }: EnhancedStatCardProps) {
  const { colors: palette } = useTheme();
  const trendColor = trend ? (trend.value > 0 ? palette.success : trend.value < 0 ? palette.error : palette.muted) : palette.muted;
  const comparisonColor = comparison ? (comparison.type === 'increase' ? palette.success : comparison.type === 'decrease' ? palette.error : palette.muted) : palette.muted;
  const isLarge = variant === 'large';
  const isCompact = variant === 'compact';

  return (
    <Animated.View entering={FadeInDown.delay(delay).springify()} style={[styles.statCard, isLarge && styles.statCardLarge, isCompact && styles.statCardCompact]}>
      <SurfaceCard style={[styles.statCardInner, isCompact && styles.statCardInnerCompact]}>
        {icon && !isCompact && (
          <View style={[styles.iconContainer, { backgroundColor: withAlpha(iconColor || palette.tint, 0.07) }, isLarge && styles.iconContainerLarge]}>
