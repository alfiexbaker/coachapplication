...
function SkillRadarChartInner({ skills, selectedSkill, onSelectSkill, showComparison, comparisonValues, comparisonLabel }: SkillRadarChartProps) {
  ...
  return (
    <Animated.View entering={FadeIn}>
      <View style={[styles.radarContainer, { width: RADAR_SIZE, height: RADAR_SIZE }]}>
        {/* Background rings */}
        {RINGS.map((ring) => (
          <View key={ring} style={[styles.ring, { width: (ring / 100) * RADIUS * 2, height: (ring / 100) * RADIUS * 2, borderColor: palette.border }]} />
        ))}
        ...
        {showComparison && comparisonValues.length === numSkills && (
          <View style={styles.polygon}>
            {skills.map((_, index) => {
              ...
              return <View key={`comp-${index}`} style={[styles.comparisonPoint, { left: pos.x - 4, top: pos.y - 4, backgroundColor: withAlpha(palette.muted, 0.25), borderColor: palette.muted }]} />;
            })}
          </View>
        )}
        ...
        <Animated.View entering={FadeInDown.springify()} style={styles.selectedDetail}>
          <View style={[styles.selectedCard, { borderColor: getSkillColor(selectedSkill.currentLevel) }]}>
            ...
            <View style={styles.selectedProgressBar}>
              ...
              <ThemedText style={[styles.selectedLevel, { color: getSkillColor(selectedSkill.currentLevel) }]}>{getSkillLabel(selectedSkill.currentLevel)}</ThemedText>
            </View>
          </View>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  radarContainer: { alignSelf: 'center', position: 'relative', justifyContent: 'center', alignItems: 'center' },
  ...
  skillLabelBg: { paddingHorizontal: Spacing.xxs, paddingVertical: Spacing.micro, borderRadius: Radii.sm },
