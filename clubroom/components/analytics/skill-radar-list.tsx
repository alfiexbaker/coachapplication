      ...
      {Object.entries(skillsByCategory).map(([category, categorySkills], catIndex) => (
        <View key={category} style={styles.categorySection}>
          <View style={styles.categorySectionHeader}>
            <ThemedText style={[styles.categorySectionTitle, { color: palette.muted }]}>{category.toUpperCase()}</ThemedText>
            <View style={[styles.categoryLine, { backgroundColor: palette.border }]} />
          </View>
          {categorySkills.map((skill, index) => {
            const skillColor = getSkillColor(skill.currentLevel);
            const changeColor = skill.changePercent > 0 ? palette.success : skill.changePercent < 0 ? palette.error : palette.muted;
            return (
              <Animated.View key={skill.skillName} entering={FadeInDown.delay((catIndex * 100) + (index * 50)).springify()} style={styles.skillRow}>
                <View style={styles.skillRowLeft}>
                  ...
                  <View style={styles.skillRowInfo}>
                    <ThemedText type="defaultSemiBold" style={styles.skillRowName}>{skill.skillName}</ThemedText>
                    <ThemedText style={[styles.skillRowLevel, { color: skillColor }]}>{getSkillLabel(skill.currentLevel)}</ThemedText>
                  </View>
                </View>
                <View style={styles.skillRowRight}>
                  <View style={styles.skillRowValue}>
                    <ThemedText type="defaultSemiBold" style={styles.skillRowValueText}>{skill.currentLevel}</ThemedText>
                  ...
                  <View style={[styles.skillRowTrend, { backgroundColor: withAlpha(changeColor, 0.09) }]}>
                    <Ionicons name={skill.changePercent > 0 ? 'trending-up' : skill.changePercent < 0 ? 'trending-down' : 'remove'} size={12} color={changeColor} />
                    <ThemedText style={[styles.skillRowTrendText, { color: changeColor }]}>{skill.changePercent > 0 ? '+' : ''}{skill.changePercent.toFixed(1)}%</ThemedText>
                  </View>
                </View>
              </Animated.View>
            );
...
const styles = StyleSheet.create({
  ...
  categorySectionHeader: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm, marginBottom: Spacing.xs },
  categorySectionTitle: { ...Typography.caption, letterSpacing: 0.5 },
  categoryLine: { flex: 1, height: 1 },
  skillRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: Spacing.xs },
  skillRowLeft: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm },
  ...
  skillRowValue: { flexDirection: 'row', alignItems: 'baseline' },
  skillRowValueText: { ...Typography.subheading },
  skillRowMax: { ...Typography.caption },
  skillRowTrend: { flexDirection: 'row', alignItems: 'center', gap: Spacing.micro, paddingHorizontal: Spacing.xxs, paddingVertical: Spacing.micro, borderRadius: Radii.sm, minWidth: 58 },
