    ...
    <Animated.View entering={FadeInDown.springify()}>
      <SurfaceCard style={styles.card}>
        <Row align="flex-start" justify="space-between">
          <View>
            <ThemedText type="defaultSemiBold" style={Typography.heading}>Squads</ThemedText>
            <ThemedText style={[Typography.small, { color: colors.muted, marginTop: Spacing.micro }]}>Organize athletes into teams</ThemedText>
          </View>
          <Clickable accessibilityLabel="Create squad" style={[styles.addBtn, { backgroundColor: colors.tint }]} onPress={onCreateSquad}>
        ...
        {squads.length === 0 ? (
          <View style={styles.empty}>
            ...
            <ThemedText style={{ color: colors.muted, textAlign: 'center' }}>No squads yet. Create one to organize your athletes.</ThemedText>
          </View>
        ) : (
          squads.map((squad) => (
            <Clickable key={squad.id} style={[styles.row, { borderColor: colors.border }]} onPress={() => router.push(Routes.clubSquad(squad.id))}>
