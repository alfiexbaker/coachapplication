...
import { OptionPicker, FocusAreaPicker, PricePreview, PackageFormFooter } from './create-package-sections';
...
export function CreatePackageForm({ onSuccess, onError, onCancel, editPackage }: CreatePackageFormProps) {
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.field}>
          <ThemedText type="defaultSemiBold" style={styles.label}>Package Name *</ThemedText>
          <TextInput style={[styles.input, { backgroundColor: palette.surface, borderColor: form.errors.name ? palette.error : palette.border, color: palette.text }]}
            placeholder="e.g., 5 Session Starter Pack" placeholderTextColor={palette.muted} value={form.name} onChangeText={form.setName} />
          {form.errors.name && <ThemedText style={[styles.errorText, { color: palette.error }]}>{form.errors.name}</ThemedText>}
        </View>

        <View style={styles.field}>
          <ThemedText type="defaultSemiBold" style={styles.label}>Description</ThemedText>
          <TextInput style={[styles.input, styles.textArea, { backgroundColor: palette.surface, borderColor: palette.border, color: palette.text }]}
            placeholder="Describe what's included..." placeholderTextColor={palette.muted} value={form.description} onChangeText={form.setDescription} multiline numberOfLines={3} />
        </View>

        <View style={styles.field}>
          <ThemedText type="defaultSemiBold" style={styles.label}>Number of Sessions</ThemedText>
        ...
        <View style={styles.field}>
          <ThemedText type="defaultSemiBold" style={styles.label}>Valid For</ThemedText>
          <OptionPicker options={validityOptions} selected={form.validDays} onSelect={form.setValidDays} />
        </View>

        <View style={styles.field}>
          <ThemedText type="defaultSemiBold" style={styles.label}>Focus Areas (optional, max 3)</ThemedText>
          <FocusAreaPicker options={FOCUS_OPTIONS} selected={form.selectedFocus} onToggle={form.toggleFocus} />
        </View>

        <View style={{ height: 100 }} />
      </ScrollView>

      <PackageFormFooter onCancel={onCancel} onSubmit={form.handleSubmit} submitting={form.submitting} isEditing={form.isEditing} />
