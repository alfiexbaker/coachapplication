...
export const OptionPicker = memo(function OptionPicker<T extends string | number>({
  options,
  selected,
  onSelect,
}: OptionPickerProps<T>) {
      ...
      {options.map((opt) => (
        <Clickable
          ...
          style={[
            styles.optionButton,
            {
              backgroundColor: selected === opt.value ? palette.tint : palette.surface,
              borderColor: selected === opt.value ? palette.tint : palette.border,
            },
          ]}
        ...
        >
          <ThemedText
            style={[
              styles.optionText,
              { color: selected === opt.value ? palette.onPrimary : palette.text },
            ]}
          >
            {opt.label}
          </ThemedText>
...
interface FocusAreaPickerProps {
  options: FootballObjective[];
  selected: FootballObjective[];
  onToggle: (f: FootballObjective) => void;
}
export const FocusAreaPicker = memo(function FocusAreaPicker({
  options,
  selected,
  onToggle,
...
export const PricePreview = memo(function PricePreview({ pricePerSession }: PricePreviewProps) {
  ...
  return (
    <SurfaceCard
      style={[styles.previewCard, { backgroundColor: withAlpha(palette.success, 0.03) }]}
    >
...
export const PackageFormFooter = memo(function PackageFormFooter({
  onCancel,
  onSubmit,
  submitting,
  isEditing,
  ...
  return (
    ...
    >
      ...
      <Clickable
        ...
        disabled={submitting}
        accessibilityRole="button"
        accessibilityLabel={isEditing ? 'Save package changes' : 'Create package'}
        accessibilityState={{ disabled: submitting }}
      >
        {submitting ? (
