...
function getDateRangeWindow(startDate: string, endDate: string): { startsAt: Date; endsAt: Date } {
...
function getBookingBusyWindows(params: {
  ...
  return getActiveRows(asRows(params.tables.bookings))
    .filter((row) => asString(row.coachUserId) === params.coachUserId)
    .filter((row) => (asString(row.status) ?? '').toUpperCase() !== 'CANCELLED')
    .map((row) => {
...
function parseGroupScheduleEntries(value: unknown): Array<{ startsAt: string; endsAt: string }> {
  return asRows(value)
    .map((row) => {
...
function getGroupSessionBusyWindows(params: {
  ...
  return getActiveRows(asRows(params.tables.groupSessions))
    .filter((row) => asString(row.coachUserId) === params.coachUserId)
    .filter((row) => {
...
function getInviteHoldBusyWindows(params: {
  ...
  return asRows(params.tables.invites)
    .filter((row) => asString(row.senderUserId) === params.coachUserId)
    .filter((row) => !asString(row.revokedAt))
    .filter((row) => (asString(row.status) ?? 'PENDING').toUpperCase() === 'PENDING')
    .filter((row) => {
      ...
      return !Number.isNaN(parsed.getTime()) && parsed.getTime() > params.now.getTime();
    })
    .filter((row) => {
    ...
    .flatMap((row) => {
      ...
      return buildInviteProposedSlotsFromMetadata(metadata)
        .map((slot) => {
...
export function resolveCoachAvailabilitySlots(params: {
  ...
  for (
    ...
    const dayTemplates = templates.filter((row) => Number(row.dayOfWeek ?? -1) === dayOfWeek);
    const override = overrides.find((row) => toDateOnly(asString(row.overrideDate)) === date);
