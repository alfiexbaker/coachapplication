...
export interface ClubScheduleActivity {
  ...
  startsAt: string;
  endsAt?: string;
  status: 'draft' | 'scheduled' | 'full' | 'in_progress' | 'cancelled' | 'completed';
...
function parseGroupScheduleEntries(value: unknown): GroupSessionScheduleEntry[] {
  return parseJsonArray(value)
    .map((entry) => (typeof entry === 'object' && entry ? (entry as GroupSessionScheduleEntry) : null))
    .filter((entry): entry is GroupSessionScheduleEntry => entry !== null);
}

function pickScheduleEntry(entries: GroupSessionScheduleEntry[], now: Date): GroupSessionScheduleEntry | null {
  if (entries.length === 0) {
    return null;
  }
...
function mapEventStatus(status: string | undefined): ClubScheduleActivity['status'] {
...
function mapEventType(metadataType: string | undefined): ClubScheduleActivity['kind'] {
...
function mapEventActivity(event: SeedRow): ClubScheduleActivity | null {
  ...
  const metadataJson = typeof event.metadataJson === 'object' && event.metadataJson ? (event.metadataJson as SeedRow) : {};
  const metadataType = asString(metadataJson.type);
  const priceMinor = asNumber(event.priceMinor);
  const squadIds = parseJsonArray(event.squadIdsJson).filter((value): value is string => typeof value === 'string');
  ...
  return {
    ...
    accessScope: squadIds.length > 0 ? 'squad' : visibility === 'public' ? 'public' : 'club',
...
function mapGroupStatus(status: string | undefined): ClubScheduleActivity['status'] {
  if (status === 'DRAFT') return 'draft';
...
function getGroupAccessScope(session: SeedRow): ClubScheduleActivity['accessScope'] {
...
function getGroupLabels(session: SeedRow): Pick<ClubScheduleActivity, 'accessLabel' | 'audienceLabel'> {
  const accessScope = getGroupAccessScope(session);
  switch (accessScope) {
    case 'mixed':
      return {
        accessLabel: asString(session.squadId) ? 'Squad + outside athletes' : 'Club + outside athletes',
    ...
    case 'squad':
      return { accessLabel: 'Squad only', audienceLabel: 'Selected squad' };
    case 'club':
      return { accessLabel: 'Club only', audienceLabel: 'Club members' };
    case 'public':
      return { accessLabel: 'Open public', audienceLabel: 'Anyone browsing sessions' };
    case 'private':
      return { accessLabel: 'Invite only', audienceLabel: 'Invited athletes only' };
  }
}

function mapGroupSessionActivity(session: SeedRow, now: Date): ClubScheduleActivity | null {
  const sessionId = asString(session.id);
  if (!sessionId) {
    return null;
  }

  const scheduleEntry = pickScheduleEntry(parseGroupScheduleEntries(session.scheduleJson), now);
  ...
  return {
    ...
    participationLabel: (asBoolean(session.waitlistEnabled) ?? false) ? 'Registration / waitlist' : 'Registration',
    accessScope: getGroupAccessScope(session),
    accessLabel: labels.accessLabel,
    audienceLabel: labels.audienceLabel,
    locationLabel: isVirtual ? 'Online' : asString(session.location) ?? 'Club training ground',
    isVirtual,
    price: typeof priceMinor === 'number' ? priceMinor / 100 : undefined,
    currency: asString(session.currency) ?? 'GBP',
    squadId,
    squadIds: squadId ? [squadId] : [],
    allowsExternalRegistration: asString(session.inviteType)?.toUpperCase() === 'OPEN',
  };
}

function mapMatchStatus(status: string | undefined): ClubScheduleActivity['status'] {
  if (status === 'COMPLETED') return 'completed';
  if (status === 'CANCELLED') return 'cancelled';
...
function mapMatchActivity(match: SeedRow): ClubScheduleActivity | null {
  ...
  const result = typeof match.resultJson === 'object' && match.resultJson ? (match.resultJson as SeedRow) : null;
  ...
  return {
    ...
    audienceLabel: squadId ? 'Selected squad and families' : 'Club fixture group',
    ...
    resultLabel: typeof home === 'number' && typeof away === 'number' ? `${home}-${away}` : undefined,
  };
}
...
export function buildClubScheduleActivities(
  ...
  const events = asRows(tables.clubEvents)
    .filter((row) => asString(row.clubId) === clubId && asString(row.status) !== 'DRAFT')
    .map(mapEventActivity)
    .filter((activity): activity is ClubScheduleActivity => activity !== null);

  const sessions = asRows(tables.groupSessions)
    .filter((row) => asString(row.clubId) === clubId && asString(row.status) !== 'DRAFT')
    .map((row) => mapGroupSessionActivity(row, now))
    .filter((activity): activity is ClubScheduleActivity => activity !== null);

  const matches = asRows(tables.matches)
    .filter((row) => asString(row.clubId) === clubId)
    .map(mapMatchActivity)
    .filter((activity): activity is ClubScheduleActivity => activity !== null);
