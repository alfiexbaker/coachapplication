...
function hydrateStoreVideoBundle(tables: SeedTables, video: SeedRow): VideoRecordBundle {
  ...
  const mediaObject = activeRows(asRows(tables.mediaObjects)).find((row) => asString(row.id) === mediaObjectId);
  if (!videoId || !mediaObject) {
    throw notFound('Video asset not found', { videoId, mediaObjectId });
  }

  return {
    video,
    mediaObject,
    annotations: activeRows(asRows(tables.videoAnnotations)).filter((row) => asString(row.videoId) === videoId),
    shares: activeRows(asRows(tables.videoShares)).filter((row) => asString(row.videoId) === videoId),
  };
}
...
function canReadVideoBundle(
  ...
  return normalizeVisibility(bundle.video.visibility) === 'SHARED' && readableShareUserIds(bundle).has(authUserId);
}

function defaultShareRecipientUserIdsFromTables(tables: SeedTables, athleteId: string | undefined): string[] {
  if (!athleteId) {
    return [];
  }
  ...
  const guardianUserIds = activeRows(asRows(tables.guardianChildLinks))
    .filter((row) => asString(row.athleteId) === athleteId)
    .map((row) => asString(row.guardianUserId))
    .filter((userId): userId is string => Boolean(userId));

  return Array.from(new Set([athleteUserId, ...guardianUserIds].filter((userId): userId is string => Boolean(userId))));
}
...
function assertFamilyAccessToAthlete(tables: SeedTables, authUserId: string, athleteId: string): void {
  const athleteUserId = asString(
    activeRows(asRows(tables.athletes)).find((row) => asString(row.id) === athleteId)?.userId,
  );
  ...
  if (!isGuardian) {
    throw forbidden('Athlete videos do not belong to authenticated user', { athleteId });
  }
}
...
class StoreVideoAuthorityRepository implements VideoAuthorityRepository {
  ...
  async listVideos(params: VideoListParams): Promise<VideoListResult> {
    ...
    if (params.coachId) {
      if (!params.isPrivilegedAdmin && params.coachId !== params.authUserId) {
        throw forbidden('Coach videos do not belong to authenticated user', { coachId: params.coachId });
      }
    ...
    const items = videos
      .map((video) => hydrateStoreVideoBundle(store.tables, video))
      .filter((bundle) => canReadVideoBundle(params.authUserId, params.isPrivilegedAdmin, bundle))
      .sort(
        (left, right) =>
  ...
  async createVideo(params: VideoCreateParams): Promise<VideoRecordResult> {
    ...
    if (!params.isPrivilegedAdmin && asString(mediaObject.ownerUserId) !== params.authUserId) {
      throw forbidden('Media object does not belong to authenticated user', { mediaObjectId: params.mediaObjectId });
    }
  ...
  async setVideoVisibility(params: VideoVisibilityParams): Promise<VideoRecordResult> {
    ...
    for (const share of activeShareRows) {
      ...
      if (!recipientUserId || !recipientUserIds.includes(recipientUserId)) {
        share.deletedAt = now;
    ...
    for (const recipientUserId of recipientUserIds) {
      const existing = videoShares.find(
        (row) => asString(row.videoId) === params.videoId && asString(row.sharedWithUserId) === recipientUserId,
      );
      if (existing) {
  ...
  async addVideoAnnotation(params: VideoAnnotationCreateParams): Promise<VideoAnnotationResult> {
    ...
    if (bundle.annotations.some((row) => asNumber(row.timestampMs) === params.timestampMs)) {
      throw badRequest('Annotation already exists at that timestamp', { videoId: params.videoId });
    }
    const durationMs = asNumber(bundle.mediaObject.durationMs);
    if (typeof durationMs === 'number' && params.timestampMs > durationMs) {
      throw badRequest('Annotation timestamp exceeds video duration', { videoId: params.videoId });
    }
  ...
  async updateVideoAnnotation(params: VideoAnnotationUpdateParams): Promise<VideoAnnotationResult> {
    ...
    if (!annotation || asString(annotation.videoId) !== params.videoId) {
      throw notFound('Video annotation not found', { annotationId: params.annotationId });
    }
    const duplicate = bundle.annotations.some(
      (row) => asString(row.id) !== params.annotationId && asNumber(row.timestampMs) === params.timestampMs,
    );
    if (duplicate) {
      throw badRequest('Annotation already exists at that timestamp', { videoId: params.videoId });
    }
  ...
  async deleteVideo(params: VideoDetailParams): Promise<void> {
    const store = this.storeProvider();
    const video = ensureRows(store.tables, 'videos').find((row) => asString(row.id) === params.videoId);
    ...
    for (const annotation of ensureRows(store.tables, 'videoAnnotations')) {
      if (asString(annotation.videoId) !== params.videoId || asString(annotation.deletedAt) != null) {
        continue;
      }
...
class PrismaVideoAuthorityRepository implements VideoAuthorityRepository {
  ...
  async listVideos(params: VideoListParams): Promise<VideoListResult> {
    ...
    return {
      items: videos
        .map(({ mediaObject, annotations, shares, ...video }) => ({
          video,
          mediaObject,
          annotations: asRows(annotations),
          shares: asRows(shares),
  ...
  async createVideo(params: VideoCreateParams): Promise<VideoRecordResult> {
    ...
    if (!created) {
      throw notFound('Video not found after creation', { mediaObjectId: params.mediaObjectId });
    }
  ...
  async setVideoVisibility(params: VideoVisibilityParams): Promise<VideoRecordResult> {
    ...
    const allowedRecipientUserIds = await this.defaultShareRecipientUserIds(asString(bundle.video.athleteId));
    const recipientUserIds =
      params.visibility === 'SHARED'
        ? assertAllowedShareRecipients(allowedRecipientUserIds, params.recipientUserIds ?? [], {
            videoId: params.videoId,
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.videoShare.updateMany({
        where: {
          videoId: params.videoId,
          deletedAt: null,
          ...(recipientUserIds.length > 0 ? { sharedWithUserId: { notIn: recipientUserIds } } : {}),
        },
        data: {
          deletedAt: now,
          updatedByUserId: params.authUserId,
  ...
  async deleteVideo(params: VideoDetailParams): Promise<void> {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.videoAnnotation.updateMany({
        where: {
          videoId: params.videoId,
          deletedAt: null,
        },
