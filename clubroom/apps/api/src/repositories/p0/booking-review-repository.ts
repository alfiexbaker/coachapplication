...
export interface BookingReviewStatusResult {
...
export interface BookingReviewRepository {
  ...
  getBookingReviewForActor(params: {
    authUserId: string;
    bookingId: string;
...
class StoreBookingReviewRepository implements BookingReviewRepository {
  ...
  async getBookingReviewForActor(params: {
    authUserId: string;
    bookingId: string;
  }): Promise<BookingReviewStatusResult> {
    ...
    const usersById = new Map(
      asRows(store.tables.users).flatMap((user) => {
        ...
        return Boolean(mapped[0]) ? [mapped] : [];
      }),
    );
    const athletesById = new Map(
      asRows(store.tables.athletes).flatMap((athlete) => {
        const mapped = [asString(athlete.id), athlete] as const;
        return Boolean(mapped[0]) ? [mapped] : [];
      }),
    );
  ...
  async listCoachReviews(params: {
    ...
    const bookingById = new Map(
      completedCoachBookings.flatMap((booking) => {
        const mapped = [asString(booking.id), booking] as const;
        return Boolean(mapped[0]) ? [mapped] : [];
      }),
    );
    ...
    const reviews = asRows(store.tables.sessionFeedback)
      .flatMap((item) => {
        if (!isPublicReviewRow(item)) return [];
        const bookingId = asString(item.bookingId);
        const booking = bookingId ? bookingById.get(bookingId) : undefined;
        const authorUserId = asString(item.authorUserId);
...
class PrismaBookingReviewRepository implements BookingReviewRepository {
  async getBookingReviewForActor(params: {
    ...
    if (shouldUseDbFixtureFallback()) {
      return dbFixtureRepository.getBookingReviewForActor(params);
    }
      ...
      return [
        {
          ...mapReviewRow(
            ...
            row.booking?.coachUserId ?? params.coachUserId,
            {
