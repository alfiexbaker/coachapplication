...
export interface CoachFavouriteRepository {
  listForUser(userId: string): Promise<{ favourites: CoachFavouriteDto[]; dataVersion: string | null }>;
  getStatus(userId: string, coachUserId: string): Promise<CoachFavouriteStatusResult>;
  saveForUser(input: {
    userId: string;
    coachUserId: string;
    note?: string | null;
  }): Promise<CoachFavouriteDto>;
  removeForUser(userId: string, coachUserId: string): Promise<CoachFavouriteDto>;
}
...
function getActiveCoachProfile(tables: SeedTables, coachUserId: string): SeedRow | undefined {
...
function getCoachDisplaySnapshot(
  ...
  CoachFavouriteDto,
  'coachName' | 'coachAvatar' | 'coachSport' | 'coachPriceMin' | 'coachPriceMax' | 'coachCity'
> {
  const user = asRows(tables.users).find((row) => asString(row.id) === coachUserId);
  const locations = asRows(tables.coachLocations).filter(
    (row) => asString(row.coachUserId) === coachUserId && !asString(row.deletedAt),
  );
  const defaultLocation = locations.find((row) => row.isDefault === true) ?? locations[0];
  const prices = asRows(tables.coachingOfferings)
    ...
    .map((row) => asNumber(row.priceMinor))
  ...
  return {
    ...
    coachPriceMax: prices.at(-1) !== undefined ? (prices.at(-1) as number) / 100 : undefined,
    coachCity: asString(defaultLocation?.label) ?? asString(defaultLocation?.addressText),
  };
}

function mapStoreFavourite(tables: SeedTables, row: SeedRow): CoachFavouriteDto {
...
function mapPrismaFavourite(row: PrismaFavouriteWithCoach): CoachFavouriteDto {
  const locations = row.coach.locations.filter((location) => !location.deletedAt);
  const defaultLocation = locations.find((location) => location.isDefault) ?? locations[0];
  const prices = row.coach.offerings
    .filter((offering) => offering.active && !offering.deletedAt)
    .map((offering) => offering.priceMinor)
    .sort((left, right) => left - right);
  ...
  return {
    ...
    coachPriceMin: prices[0] !== undefined ? prices[0] / 100 : undefined,
    coachPriceMax: prices.at(-1) !== undefined ? (prices.at(-1) as number) / 100 : undefined,
    coachCity: defaultLocation?.label ?? defaultLocation?.addressText ?? undefined,
  };
}
...
class StoreCoachFavouriteRepository implements CoachFavouriteRepository {
  ...
  async listForUser(userId: string): Promise<{ favourites: CoachFavouriteDto[]; dataVersion: string | null }> {
    const store = this.storeProvider();
    const favourites = asRows(store.tables.coachFavourites)
      .filter((row) => asString(row.userId) === userId && isActiveFavourite(row))
      .map((row) => mapStoreFavourite(store.tables, row))
      .sort((left, right) => toSortTime(right.createdAt) - toSortTime(left.createdAt));

    return { favourites, dataVersion: store.version };
  }

  async getStatus(userId: string, coachUserId: string): Promise<CoachFavouriteStatusResult> {
    const store = this.storeProvider();
    const row = asRows(store.tables.coachFavourites).find(
      (candidate) =>
        asString(candidate.userId) === userId &&
        (asString(candidate.coachUserId) === coachUserId || asString(candidate.coachId) === coachUserId),
    );
    if (!row || !isActiveFavourite(row)) {
      return { isFavourite: false, favourite: null };
    }
    return { isFavourite: true, favourite: mapStoreFavourite(store.tables, row) };
  }
  ...
  async saveForUser(input: {
    ...
    if (!getActiveCoachProfile(store.tables, input.coachUserId)) {
      throw notFound('Coach not found', { coachId: input.coachUserId });
    }
  ...
  async removeForUser(userId: string, coachUserId: string): Promise<CoachFavouriteDto> {
    const store = this.storeProvider();
    const rows = ensureTable(store.tables, 'coachFavourites');
    const row = rows.find(
      (candidate) =>
    ...
    if (!row || !isActiveFavourite(row)) {
      throw notFound('Favourite not found', { coachId: coachUserId });
    }
...
class PrismaCoachFavouriteRepository implements CoachFavouriteRepository {
  private readonly fallback = new StoreCoachFavouriteRepository(() => getDbFixtureStore());
  ...
  async listForUser(userId: string): Promise<{ favourites: CoachFavouriteDto[]; dataVersion: string | null }> {
    ...
    const rows = await prisma.coachFavourite.findMany({
      where: { userId, isFavourite: true, deletedAt: null },
  ...
  async getStatus(userId: string, coachUserId: string): Promise<CoachFavouriteStatusResult> {
    ...
    const row = await prisma.coachFavourite.findUnique({
      where: { userId_coachUserId: { userId, coachUserId } },
      include: this.includeCoachSnapshot,
    });
    if (!row || !row.isFavourite || row.deletedAt) {
      return { isFavourite: false, favourite: null };
    }
    return { isFavourite: true, favourite: mapPrismaFavourite(row) };
  }
  ...
  async saveForUser(input: {
    ...
    const coach = await prisma.coachProfile.findFirst({
      where: { userId: input.coachUserId, deletedAt: null },
      select: { userId: true },
    });
    if (!coach) {
      throw notFound('Coach not found', { coachId: input.coachUserId });
    ...
    const existing = await prisma.coachFavourite.findUnique({
      where: { userId_coachUserId: { userId: input.userId, coachUserId: input.coachUserId } },
    });
...
const seedRepository = new StoreCoachFavouriteRepository(() => getMarketplaceSeedStore());
