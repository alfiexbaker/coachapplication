...
function buildCornerScores(skills: Record<string, SkillLevelLike>): CornerScoreMap {
...
export function useCornerPercentiles({
  ...
  const cornerValuesKey = useMemo(
    () =>
      `${cornerValues.technical}|${cornerValues.physical}|${cornerValues.psychological}|${cornerValues.social}`,
    [
      cornerValues.physical,
      cornerValues.psychological,
      cornerValues.social,
      cornerValues.technical,
    ],
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const allSkillLevels = await apiClient.get<Record<string, AthleteSkillLevelsLike>>(
        ...
        (Object.keys(next) as FourCornerKey[]).forEach((corner) => {
          ...
          const pool = peerCornerScores
            .map((score) => score[corner])
          ...
          const higherCount = pool.filter((score) => score > athleteScore).length;
          const rank = higherCount + 1;
          const topPercent = Math.max(1, Math.min(100, Math.round((rank / pool.length) * 100)));
