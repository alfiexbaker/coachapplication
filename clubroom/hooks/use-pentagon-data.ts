...
function toTrend(
  trend: SkillLevel["trend"] | null | undefined,
): SkillTrendDirection {
  if (trend === "improving") {
    return "improving";
  }
...
export function usePentagonData(
  ...
  const [comparisonTargetTimestamp] = useState(
    () => Date.now() - COMPARISON_WINDOW_MS,
  );
  return (() => {
    ...
    const attributes: PentagonAttribute[] = positionSkills.map((skill) => {
      ...
      const level = Math.max(
        0,
        Math.min(10, Math.round(levelEntry?.level ?? 0)),
      );
    ...
    const universalSkills: UniversalSkillRating[] = UNIVERSAL_SKILLS.map(
      (skill) => {
        const levelEntry = skillMap.get(skill.toLowerCase());
        const level = Math.max(
          0,
    ...
    const positionFeedback = feedback
      .filter(
        (entry) =>
          entry.positionPlayed === position && entry.skillRatings.length > 0,
      )
    ...
    const snapshots = positionFeedback
      ...
      .reverse()
      .map((entry) => {
        const values = attributes.reduce<Record<string, number>>(
          (acc, attribute) => {
    ...
    const currentValues = attributes.reduce<Record<string, number>>(
      (acc, attribute) => {
    ...
    const shouldAppendCurrent =
      ...
      attributes.some(
        (attribute) =>
          Math.abs(
            (latestSnapshot.values[attribute.key] ?? 0) -
    ...
    const comparisonFeedback =
      positionFeedback.find(
        (entry) =>
          new Date(entry.createdAt).getTime() <= comparisonTargetTimestamp,
      ) ?? positionFeedback[positionFeedback.length - 1];
    const deltas = attributes.reduce<Record<string, number>>(
      (acc, attribute) => {
        const currentRating = getSkillRating(newestFeedback, attribute.label);
        const previousRating = getSkillRating(
          comparisonFeedback,
          attribute.label,
        );
    ...
    const comparisonLabel =
      newestFeedback &&
      comparisonFeedback &&
      newestFeedback.id !== comparisonFeedback.id
        ? formatShortDate(comparisonFeedback.createdAt)
        : null;
    const availablePositions = (["MID", "DEF", "GK", "ATT"] as const).flatMap(
      (role) => {
        const mapped = {
          role,
          sessionCount: feedback.filter(
            (entry) => entry.positionPlayed === role,
          ).length,
        };
        return mapped.sessionCount > 0 ? [mapped] : [];
      },
    );
