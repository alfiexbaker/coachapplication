...
async function getBookingsInRange(coachId: string, startDate: string, endDate: string): Promise<{ count: number; dates: string[] }> {
  try {
...
export function useBlockedDates(coachId: string, onUpdate?: (blockedDates: BlockedDateRange[]) => void) {
  const [loading, setLoading] = useState(true);
  const [blockedDates, setBlockedDates] = useState<BlockedDateRange[]>([]);
  const [selectedStart, setSelectedStart] = useState<string | null>(null);
  const [selectedEnd, setSelectedEnd] = useState<string | null>(null);
  const [reason, setReason] = useState('');
  const [bookingConflict, setBookingConflict] = useState<{ count: number; dates: string[] }>({ count: 0, dates: [] });

  useEffect(() => {
    (async () => { setBlockedDates(await loadBlockedDates(coachId)); setLoading(false); })();
  }, [coachId]);

  useEffect(() => {
    if (!selectedStart) { setBookingConflict({ count: 0, dates: [] }); return; }
    const start = selectedEnd ? (selectedStart <= (selectedEnd ?? selectedStart) ? selectedStart : selectedEnd) : selectedStart;
    const end = selectedEnd ? (selectedStart <= selectedEnd ? selectedEnd : selectedStart) : selectedStart;
    (async () => { setBookingConflict(await getBookingsInRange(coachId, start, end)); })();
  }, [coachId, selectedStart, selectedEnd]);

  const handleDateSelect = useCallback((date: string) => {
    if (!selectedStart || (selectedStart && selectedEnd)) { setSelectedStart(date); setSelectedEnd(null); }
    else { setSelectedEnd(date); }
  }, [selectedStart, selectedEnd]);

  const handleAddBlock = useCallback(async () => {
    if (!selectedStart) return;
    const start = selectedEnd && selectedEnd < selectedStart ? selectedEnd : selectedStart;
    const end = selectedEnd ? (selectedEnd > selectedStart ? selectedEnd : selectedStart) : selectedStart;
    const newBlock: BlockedDateRange = {
      id: `block_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`,
      coachId, startDate: start, endDate: end, reason: reason.trim(), createdAt: new Date().toISOString(),
    };
    const updated = [...blockedDates, newBlock].sort((a, b) => a.startDate.localeCompare(b.startDate));
    setBlockedDates(updated);
    await saveBlockedDates(coachId, updated);
    setSelectedStart(null); setSelectedEnd(null); setReason('');
  ...
  const doBlockWeek = useCallback(async (start: string, end: string) => {
    const newBlock: BlockedDateRange = {
      id: `block_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`,
      coachId, startDate: start, endDate: end, reason: 'Blocked entire week', createdAt: new Date().toISOString(),
    };
    const updated = [...blockedDates, newBlock].sort((a, b) => a.startDate.localeCompare(b.startDate));
    setBlockedDates(updated); await saveBlockedDates(coachId, updated);
    onUpdate?.(updated); logger.debug('Week blocked', { start, end });
  ...
  const handleBlockThisWeek = useCallback(async () => {
    ...
    if (conflicts.count > 0) {
      Alert.alert('Bookings exist this week',
        `You have ${conflicts.count} booking${conflicts.count > 1 ? 's' : ''} this week. Blocking will not cancel them. Continue?`,
        [{ text: 'Cancel', style: 'cancel' }, { text: 'Block anyway', onPress: () => doBlockWeek(start, end) }]);
    } else { await doBlockWeek(start, end); }
  }, [coachId, doBlockWeek]);
  ...
  const handleRemoveBlock = useCallback((blockId: string) => {
    Alert.alert('Remove blocked dates?', 'Sessions will be bookable again for these dates.', [
      ...
      { text: 'Remove', style: 'destructive', onPress: async () => {
        ...
        setBlockedDates(updated); await saveBlockedDates(coachId, updated); onUpdate?.(updated);
      }},
    ]);
  }, [coachId, blockedDates, onUpdate]);
  ...
  const selectionLabel = selectedStart
    ? selectedEnd
      ? formatDateRange(selectedStart <= selectedEnd ? selectedStart : selectedEnd, selectedStart <= selectedEnd ? selectedEnd : selectedStart)
