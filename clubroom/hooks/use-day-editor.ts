...
export function useDayEditor({
  ...
  useEffect(() => {
    ...
    if (template) {
      ...
      const sorted = [...existingTemplatesForDay].sort((a, b) => a.endTime.localeCompare(b.endTime));
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gesture) => { if (gesture.dy > 0) slideAnim.value = gesture.dy; },
  ...
  const overlapWarning = useMemo(() => {
    ...
    const siblings = template ? existingTemplatesForDay.filter((t) => t.id !== template.id) : existingTemplatesForDay;
  ...
  }, [isValid, scope, dayOfWeek, startTime, endTime, location, dateStr, onSaveRecurring, onSaveOverride]);
  ...
  return {
    dayName, scope, startTime, endTime, location, setLocation,
    showAddVenueInput, setShowAddVenueInput, newVenueLabel, setNewVenueLabel,
    slideAnim, overlayOpacity, panResponder,
    isValid, overlapWarning, isNewTimeBlock, durationLabel, formattedDate, saveLabel,
    handleStartTimeChange, setEndTime, handleSave, handleDelete, handleAddVenue, handleScopeChange,
  };
}
