...
export function useInviteSessionFlow({ visible, coachId, onClose, onComplete }: UseInviteSessionFlowProps) {
  ...
  const loadUpcomingSessions = useCallback(async () => {
    try {
      ...
      const now = new Date();
      setUpcomingSessions(
        bookings.filter(b => new Date(b.scheduledAt) > now && b.coachId === coachId)
          .sort((a, b) => new Date(a.scheduledAt).getTime() - new Date(b.scheduledAt).getTime())
          .slice(0, 10)
      );
    } catch (error) { logger.error('Failed to load upcoming sessions', error); }
  }, [coachId]);

  useEffect(() => { if (visible) loadUpcomingSessions(); }, [visible, loadUpcomingSessions]);

  const handleChoiceSelect = useCallback((choice: 'existing' | 'new') => {
    void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    logger.action('InviteChoiceSelected', { choice });
    if (choice === 'new') { setIsNewSession(true); setStep('select-athletes'); return; }
    setIsNewSession(false);
    if (upcomingSessions.length === 0) {
      Alert.alert('No Upcoming Sessions', "You don't have any upcoming sessions. Would you like to create a new one?", [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Create New', onPress: () => { setIsNewSession(true); setStep('select-athletes'); } },
      ]);
    } else { setStep('select-session'); }
  ...
  const handleAthletesSelected = useCallback((selected: Athlete[]) => {
    ...
    } else if (selectedSession) { setStep('confirm'); }
  }, [isNewSession, selectedSession, onClose]);

  const handleConfirm = useCallback(async () => {
    if (!selectedSession || selectedAthletes.length === 0) return;
    void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    logger.action('InviteConfirmed', { sessionId: selectedSession.id, athleteCount: selectedAthletes.length });
    try {
      const bookings = await apiClient.get<UpcomingSession[]>('coach_bookings', []);
      const updatedBookings = bookings.map(b => {
        if (b.id === selectedSession.id) {
          ...
          return { ...b, athleteIds: [...new Set([...existingIds, ...newIds])], currentAthletes: (b.currentAthletes || 0) + selectedAthletes.length };
        }
        return b;
      });
      await apiClient.set('coach_bookings', updatedBookings);
      onComplete?.({ sessionId: selectedSession.id, athleteIds: selectedAthletes.map(a => a.id), isNew: false });
      Alert.alert('Athletes Invited', `${selectedAthletes.length} athlete${selectedAthletes.length !== 1 ? 's' : ''} added to ${selectedSession.title || 'session'}.`);
      handleClose();
    } catch (error) {
  ...
  const handleClose = useCallback(() => {
    setStep('choice'); setSelectedSession(null); setSelectedAthletes([]); setIsNewSession(false); onClose();
  }, [onClose]);
  ...
  return {
    step, upcomingSessions, selectedSession, selectedAthletes, isNewSession,
    handleChoiceSelect, handleSessionSelect, handleAthletesSelected, handleConfirm, handleClose, handleBack,
  };
}
