...
export function useDrillForm({ initialValues, onSubmit }: UseDrillFormOptions) {
  ...
  const validate = useCallback((): boolean => {
    ...
    if (isNaN(durationNum) || durationNum < 1 || durationNum > 180) newErrors.duration = 'Duration must be between 1 and 180 minutes';
  ...
  const handleSubmit = useCallback(() => {
    ...
    const values: CreateDrillInput = {
      ...
      category, difficulty, duration: parseInt(duration, 10),
      videoUrl: videoUrl.trim() || undefined,
      equipment: equipment.trim() ? equipment.split(',').map((e) => e.trim()).filter(Boolean) : undefined,
  ...
  return {
    ...
    duration, setDuration, videoUrl, setVideoUrl,
    equipment, setEquipment, tags, setTags,
    errors, handleSubmit,
  };
}
