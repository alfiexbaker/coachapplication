...
export function useGroupRoster(sessionId: string | undefined) {
  ...
  const rsvpCounts: RosterRsvpCounts = (() => {
    const counts = {
      going: 0,
      maybe: 0,
      notGoing: 0,
  ...
  const getRsvpForRegistration = (registration: GroupRegistration): SessionRsvp | null => {
    return (
      rsvps.find(
        (rv) =>
  ...
  const startRollCall = () => {
    ...
    roster.forEach((r) => {
      if (!(r.status === 'REGISTERED' || r.status === 'ATTENDED')) return;
      initial[r.id] = r.status === 'ATTENDED' ? 'present' : 'unmarked';
    });
    setRollCallAttendance(initial);
    setShowRollCall(true);
  };
  const markRollCallStatus = (id: string, status: AttendanceStatus) => {
  ...
  const saveRollCall = async () => {
    ...
    try {
      const attendanceResults = await Promise.all(
        Object.entries(rollCallAttendance).map(async ([registrationId, status]) => {
      ...
      const failedAttendanceResult = attendanceResults.find((result) => result?.success === false);
      if (failedAttendanceResult?.success === false) {
        uiFeedback.showToast(
          failedAttendanceResult.error.message || 'Failed to save roll call. Please try again.',
          'error',
        );
        return;
      }
