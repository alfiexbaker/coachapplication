  ...
  async notifyAssignmentChange(params: {
    ...
    try {
      ...
      if (
        ...
        params.previousAssigneeId !== params.nextAssigneeId
      ) {
        notifyTasks.push(
          notificationService.notifyCoachBookingHandoff({
      ...
      params.updatedBookings.forEach((booking) => {
        if (
          !booking.bookedById ||
          parentNotifications.has(booking.bookedById)
        ) {
          return;
        }
        ...
        const relationshipContext = getBookingRelationshipContext({
          ...
          organizationLabel,
          coachLabel:
            params.previousAssigneeName ||
            booking.coachName ||
            params.nextAssigneeName,
      ...
      logger.error("Failed to send assignment change notifications", {
        params,
        error,
      });
  ...
  async notifySupportIssueReported(params: {
    ...
    try {
      ...
      if (
        ...
        booking.clubId &&
        booking.commercialMode === "ORG_OWNED"
      ) {
        const memberships = await socialFeedService.getClubMemberships(
          booking.clubId,
        );
        memberships.forEach((membership) => {
          if (
            !(
              ...
              canManageClubAssignments(membership.role)
            )
          )
            return;
          return recipients.add(membership.userId);
        });
      }
      if (recipients.size === 0) {
        const supportCoachId =
          booking.assigneeCoachId || booking.ownerCoachId || booking.coachId;
        if (supportCoachId) {
      ...
      logger.error("Failed to notify support issue", {
        params,
        error,
      });
