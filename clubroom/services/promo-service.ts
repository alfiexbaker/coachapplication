...
const logger = createLogger('PromoService');
...
const MOCK_PROMO_USAGE: PromoCodeUsage[] = [
  {
    id: 'usage_1',
 ...
 */
function generatePromoCodeId(): string {
 ...
 */
function isCodeExpired(code: PromoCode): boolean {
 ...
 */
function isCodeExhausted(code: PromoCode): boolean {
 ...
 */
function getCodeStatus(code: PromoCode): 'active' | 'expired' | 'exhausted' | 'inactive' {
  if (!code.isActive) return 'inactive';
  if (isCodeExpired(code)) return 'expired';
  if (isCodeExhausted(code)) return 'exhausted';
  return 'active';
}
 ...
 */
async function getAllPromoCodes(): Promise<PromoCode[]> {
  const codes = await getAllCodes();
 ...
 */
async function getPromoCodeById(codeId: string): Promise<PromoCode | null> {
  const codes = await getAllCodes();
 ...
 * @returns The promo code or null if not found
 */
async function getPromoCodeByString(code: string): Promise<PromoCode | null> {
  const codes = await getAllCodes();
 ...
 */
async function getCodeUsage(codeId: string): Promise<PromoCodeUsage[]> {
  const usage = await getAllUsage();
  return usage
 ...
 * @returns Array of usage records
 */
async function getUserUsage(userId: string): Promise<PromoCodeUsage[]> {
  const usage = await getAllUsage();
  return usage
 ...
 */
async function deactivateCode(codeId: string): Promise<PromoCode | null> {
  const codes = await getAllCodes();
 ...
 * @returns The updated promo code or null if not found
 */
async function reactivateCode(codeId: string): Promise<PromoCode | null> {
  const codes = await getAllCodes();
  const codeIndex = codes.findIndex((c) => c.id === codeId);
 ...
 */
async function deletePromoCode(codeId: string): Promise<boolean> {
  const codes = await getAllCodes();
  const codeIndex = codes.findIndex((c) => c.id === codeId);
 ...
 */
async function getCodeStats(): Promise<PromoCodeStats> {
 ...
 */
async function getCodeDetailedStats(codeId: string): Promise<{
  code: PromoCode;
 ...
 */
function getStatusInfo(status: 'active' | 'expired' | 'exhausted' | 'inactive'): {
  label: string;
  color: string;
} {
  switch (status) {
