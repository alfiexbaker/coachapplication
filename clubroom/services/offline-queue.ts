 ...
 *
 * Queues mock-mode write operations when offline, flushes on reconnect.
 * Queue persists across app restart via AsyncStorage only in mock/test mode.
 *
...
const API_MODE_DISABLED_MESSAGE =
  'Offline queue is mock-only. API mode requires explicit /v1 flows to retry writes against backend authority.';
...
async function saveQueue(queue: QueuedAction[]): Promise<Result<void, ServiceError>> {
  if (!apiClient.isMockMode) {
    ...
    return err(unsupportedError(API_MODE_DISABLED_MESSAGE));
  }
...
async function clearLegacyApiModeQueue(): Promise<void> {
  try {
    await apiClient.removeLocal(STORAGE_KEYS.OFFLINE_QUEUE);
  } catch (error) {
    logger.warn('Failed to clear legacy API-mode offline queue', error);
  }
}
...
export async function addToQueue(
  ...
  if (!apiClient.isMockMode) {
    await clearLegacyApiModeQueue();
    logger.warn('Blocked API-mode offline queue write', {
      path: action.path,
...
export async function clearQueue(): Promise<Result<void, ServiceError>> {
  if (!apiClient.isMockMode) {
    await clearLegacyApiModeQueue();
    logger.info('Legacy API-mode offline queue cleared');
    return ok(undefined);
  }
...
export function initAutoFlush(): () => void {
  if (!apiClient.isMockMode) {
    clearLegacyApiModeQueue().catch((error) =>
      logger.warn('Failed to clear legacy API-mode offline queue during init', error),
    );
    return () => {};
  }
