...
let registrationsCache: GroupRegistration[] = isMockMode() ? [...MOCK_REGISTRATIONS] : [];
    ...
    if (isMockMode()) {
      return withRegistrationLock(async () => {
        ...
        const existing = registrationsCache.find(
          (r) => r.sessionId === sessionId && r.athleteId === athleteId && r.status !== 'CANCELLED',
        );
        if (existing) {
          ...
          return err(
            serviceError('CONFLICT', 'This athlete is already registered for this session.'),
          );
        }
        ...
        if (!isFull) {
          return err(
            serviceError('CONFLICT', 'This session still has spaces available. Register instead.'),
          );
        }
        if (!session.waitlistEnabled) {
          return err(serviceError('VALIDATION', 'This session does not have a waitlist.'));
        }
