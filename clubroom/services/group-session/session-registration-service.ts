...
export const sessionRegistrationService = {
  ...
  async register(
    ...
    if (USE_MOCK) {
      ...
      return withRegistrationLock(async () => {
        ...
        if (!isFull) {
          const athleteDisplayName = await resolveUserName(
            athleteId,
            "Athlete",
          );
        ...
        if (!isFull) {
          try {
            const [coachDisplayName, athleteDisplayName, parentDisplayName] =
              await Promise.all([
            ...
            if (bookingResult.success) {
              ...
              const linkResult = await bookingCrudService.updateBooking(
                booking.id,
                {
                  isGroupSession: true,
            ...
            logger.error(
              "Failed to create linked booking for group registration",
              error,
            );
          }
        }
  ...
  async cancelRegistration(
    ...
    if (USE_MOCK) {
      ...
      const sessionForNotif = sessionsCache.find(
        (s) => s.id === registration.sessionId,
      );
      ...
      const session = sessionsCache.find(
        (s) => s.id === registration.sessionId,
      );
      if (session) {
        if (wasRegistered) {
    ...
    const result =
      await groupSessionAuthorityService.cancelRegistration(registrationId);
    if (!result.success) {
      return err(result.error);
    }
    return ok(undefined);
  ...
  async markAttendance(
    ...
    if (USE_MOCK) {
      ...
      if (attended) {
        ...
        registration.attendedDates = registration.attendedDates.filter(
          (d) => d !== date,
        );
   ...
   */
  async getParentRegistrations(parentId: string): Promise<
    (GroupRegistration & {
  ...
  > {
    if (USE_MOCK) {
      ...
      return registrationsCache
        .flatMap((r) => {
          if (!(r.parentId === parentId && r.status !== "CANCELLED")) return [];
          const mapped = {
            ...r,
            session: sessionsCache.find((s) => s.id === r.sessionId)!,
          };
          return mapped.session ? [mapped] : [];
        })
        .sort((a, b) => {
  ...
  async getRegistrationsForAthletes(
    ...
    const result =
      await groupSessionAuthorityService.getRegistrationsForAthletes(
        athleteIds,
      );
    if (!result.success) {
      throw new Error(result.error.message);
