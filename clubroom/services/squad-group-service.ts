 ...
 */
async function saveMap(map: SquadGroupMap): Promise<Result<void, ServiceError>> {
  try {
...
export const squadGroupService = {
  ...
  async getOrCreateSquadGroup(
    ...
    try {
      ...
      try {
        const storedSquads = await apiClient.get<Array<{ id: string; groupId?: string }>>(
  ...
  async syncMemberRemovalFromGroup(
    ...
    const result = await communityGroupService.removeMemberDirect(groupId, parentId);
