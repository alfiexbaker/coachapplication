...
function sortTasksForAthlete(left: PracticeTask, right: PracticeTask): number {
  ...
  if (
    !Number.isNaN(leftAssigned) &&
    !Number.isNaN(rightAssigned) &&
...
async function updateAssignmentDueDate(assignmentId: string, dueAtIso: string): Promise<boolean> {
...
function buildTaskFromDrillAssignment(
  ...
  return {
    ...
    completedByUserId: assignment.isCompleted
      ? (existing?.completedByUserId ?? assignment.athleteId)
      : undefined,
    completionNote: assignment.isCompleted
      ? assignment.evidenceNotes?.trim() ||
        assignment.athleteFeedback?.trim() ||
...
async function syncPracticeTasks(): Promise<PracticeTaskRecord[]> {
  ...
  const taskByFeedbackId = new Map(
    existingTasks.flatMap((task) =>
      task.source === 'session_feedback' ? [[task.sourceFeedbackId, task]] : [],
    ),
  );
  const taskByDrillAssignmentId = new Map(
    existingTasks.flatMap((task) =>
      task.source === 'drill_assignment'
        ? [[task.drillAssignmentId ?? task.sourceFeedbackId, task]]
        : [],
    ),
  );
...
async function listTasksForAthlete(
  ...
  return tasks
    .flatMap((task) => {
      if (!(task.athleteId === athleteId)) return [];
      return canViewerAccessTask(task, viewerRole) ? [toViewModel(task, nowTs)] : [];
    })
...
async function setRecoveryCheckpoint(
  ...
  try {
    ...
    logger.error('Failed to set recovery checkpoint', {
      taskIds,
      actorId,
      hours,
      error,
...
async function listCoachFollowUpQueue(coachId: string): Promise<CoachFollowUpItem[]> {
  ...
  for (const [athleteId, athleteTasks] of groupedByAthlete.entries()) {
    const overdueCount = athleteTasks.filter(
      (task) => deriveTaskTiming(task, nowTs) === 'overdue',
    ).length;
    const dueSoonCount = athleteTasks.filter(
      (task) => deriveTaskTiming(task, nowTs) === 'due_soon',
    ).length;
    const nextDueAt = athleteTasks.reduce<string | null>((earliest, task) => {
      if (Number.isNaN(parseTimestamp(task.dueAt))) {
        return earliest;
      }
      return earliest === null || parseTimestamp(task.dueAt) < parseTimestamp(earliest)
        ? task.dueAt
        : earliest;
    }, null);
    ...
    const latestCoachActionAt = athleteTasks.reduce<string | null>((latest, task) => {
