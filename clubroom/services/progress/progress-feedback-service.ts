...
async function addSessionFeedback(
  ...
  const existingIndex = allFeedback.findIndex(
    (entry) =>
      entry.athleteId === feedback.athleteId &&
      entry.sessionId === feedback.sessionId,
  );
  const now = new Date().toISOString();
  const previousFeedback =
    existingIndex >= 0 ? allFeedback[existingIndex] : null;
  const nextFeedback: SessionFeedback = previousFeedback
    ? {
        ...
        positionPlayed:
          feedback.positionPlayed ?? previousFeedback.positionPlayed,
        positionsPlayed:
          feedback.positionsPlayed ?? previousFeedback.positionsPlayed,
        subSkillRatings:
          feedback.subSkillRatings ?? previousFeedback.subSkillRatings,
  ...
  const shouldUpdateSkills =
    ...
    JSON.stringify(previousFeedback?.skillRatings ?? []) !==
      JSON.stringify(feedback.skillRatings);
...
async function createFeedbackFromQuickRate(
  ...
  athleteName: string,
  options?: {
    skillsAlreadyWritten?: boolean;
  },
): Promise<Result<SessionFeedback, ServiceError>> {
  try {
    const positionSkillRatings = (input.positionSkillRatings ?? []).flatMap(
      (entry) =>
        Boolean(entry?.skill)
          ? [
              {
                ...entry,
                rating: Math.max(1, Math.min(5, Math.round(entry.rating))) as
                  | 1
                  | 2
                  | 3
                  | 4
    ...
    const fourCorners: FourCornerRatings =
      input.positionPlayed && positionSkillRatings.length > 0
        ...
        : {
            technical: Math.max(
              1,
              Math.min(5, Math.round(input.technical ?? 3)),
            ),
            physical: Math.max(1, Math.min(5, Math.round(input.physical ?? 3))),
            psychological: Math.max(
              1,
              Math.min(5, Math.round(input.psychological ?? 3)),
            ),
            social: Math.max(1, Math.min(5, Math.round(input.social ?? 3))),
          };
    const previousCorners = await getPreviousCorners(
      input.athleteId,
      input.coachId,
    );
    const latestForSession = await getLatestForAthlete(
      input.athleteId,
      input.sessionId,
    );
    const existingForSession =
      latestForSession?.coachId === input.coachId ? latestForSession : null;
    const cornerSkillsWorkedOn = (
      Object.keys(fourCorners) as FourCornerKey[]
    ).filter((key) =>
      previousCorners ? fourCorners[key] !== previousCorners[key] : true,
    );
    const focusSkills = Array.from(
      new Set(
        (input.focusSkills ?? []).flatMap((skill) => {
          const mapped = skill.trim();
          return mapped.length > 0 ? [mapped] : [];
        }),
      ),
    );
    const generatedSummary = generateQuickRateSummary(
      fourCorners,
      previousCorners,
      1,
    );
    ...
    const feedback = await addSessionFeedback(
      {
        ...
        skillsWorkedOn:
          ...
          existingForSession.skillsWorkedOn.length > 0
            ? existingForSession.skillsWorkedOn
        ...
        effortRating: Number.isFinite(input.effort)
          ? Math.max(1, Math.min(5, Math.round(input.effort)))
          : 3,
        overallPerformance:
          existingForSession?.overallPerformance ??
          (input.overallPerformance != null &&
          Number.isFinite(input.overallPerformance)
            ? Math.max(
                1,
                Math.min(5, Math.round(input.overallPerformance as number)),
              )
            : undefined) ??
        ...
        visibility:
          existingForSession?.visibility ?? input.visibility ?? "athlete",
        fourCorners,
        positionPlayed:
          input.positionPlayed ?? existingForSession?.positionPlayed,
        positionsPlayed:
          input.positionsPlayed ?? existingForSession?.positionsPlayed,
        subSkillRatings:
          input.subSkillRatings ?? existingForSession?.subSkillRatings,
        sessionTemplateId:
          existingForSession?.sessionTemplateId ?? input.sessionTemplateId,
        sessionTemplateName:
          existingForSession?.sessionTemplateName ?? input.sessionTemplateName,
