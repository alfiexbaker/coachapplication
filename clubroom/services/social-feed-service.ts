...
async function resolveFeedApiContext(
  message: string,
): Promise<Result<FeedApiContext, ServiceError>> {
...
function getUserClubsInternal(userId: string): Club[] {
  ...
  return memberships.flatMap((membership) => {
    const mapped = getClubById(membership.clubId);
...
class ClubFeedService {
  ...
  private async notifyClubMembers(
    ...
    await Promise.all(
      recipients.map((memberId) =>
        notificationService.notifyParentClubPost({
          parentId: memberId,
          clubName,
          postId,
          clubId,
  ...
  leaveClub(userId: string, clubId: string): boolean {
    ...
    void this.persistMemberships();
    emitTyped(ServiceEvents.CLUB_MEMBER_LEFT, {
      clubId,
      userId,
    });
    ...
    if (targetInvite.role === "MEMBER") {
      ...
      clubsStore = clubsStore.map((club) =>
        club.id === clubId
          ? {
              ...club,
              inviteCode: fallbackInvite.code,
            }
          : club,
      );
      await this.persistClubs();
  ...
  async createCoachPostAuthority(
    input: CreateCoachPostInput,
  ): Promise<Result<ClubFeedPost, ServiceError>> {
    if (USE_MOCK) {
