...
import { orgHeadCoachService, type HeadCoachCoachHealth, type HeadCoachCompletionItem } from '@/services/org-head-coach-service';
import { orgStaffingService, type OrgWorkItem } from '@/services/org-staffing-service';
import { type Result, type ServiceError, err, ok, storageError } from '@/types/result';
...
class OrgOwnerDashboardService {
  async getDashboardData(
    ...
    try {
      const [staffingResult, oversightResult, bookings, offerings, invoices, problemReports] =
        await Promise.all([
      ...
      const clubOfferings = offerings.filter(
        (offering) => offering.actingAs === 'club' && offering.clubId === clubId,
      );
      const invoiceByBookingId = new Map(invoices.map((invoice) => [invoice.bookingId, invoice]));
      ...
      clubBookings
        ...
        .forEach((booking) => {
          ...
          applyInvoiceToSummary(finance, booking, invoice, booking.scheduledAt);
        });
      ...
      const bookingById = new Map(clubBookings.map((booking) => [booking.id, booking]));
      ...
      const coachHealth = [...oversight.coachHealth].sort((left, right) => {
      ...
      return ok({
        ...
        summary: {
          activeStaffCount: staffing.staff.filter((member) => member.status === 'active').length,
      ...
      logger.error('Failed to build owner dashboard', { clubId, viewerUserId, error });
