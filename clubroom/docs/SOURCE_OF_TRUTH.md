...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - community group message send/read transitions, notification read/dismiss/clear state, and notification preferences are backend-authoritative in API mode: active group membership or notification ownership is required, the actor comes from auth, and local message/read/notification preference overlays are mock-only for these transitions
