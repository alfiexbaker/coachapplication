...
- Club join, invite-code management, pending staff invite review, join-link resolution, and club member management now use `/v1/clubs/*` in non-mock mode through the club authority runtime.
  ...
  - club member list, role update, soft-remove, ban, removal undo, and squad assignment now use `GET /v1/clubs/:clubId/members`, `PATCH /v1/clubs/:clubId/members/:userId/role`, `DELETE /v1/clubs/:clubId/members/:userId`, `POST /v1/clubs/:clubId/members/:userId/ban`, `POST /v1/clubs/:clubId/members/removals/:removalId/restore`, and `PUT/DELETE /v1/clubs/:clubId/squads/:squadId/members/:userId`; member mutations require `manage_staff_and_invites`, cannot assign/remove/restore ownership, ban writes block future join-code revival, resolve squad assignment through linked athlete `SquadMembership`, and write audit events for success and deny paths
...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - coach profile in non-mock mode now reads its own offerings from `/v1/coaches/me/offerings`; public coach profile, booking session-type/schedule, and Discover Map booking fast-track read target coach active offerings from `/v1/coaches/:coachId/offerings`; Discover Map coach-card pricing/bookability summaries read active offering truth from `/v1/coaches/offerings`; go-live state writes through `PATCH /v1/auth/me` instead of local-only toggles
  ...
  - payout method and withdrawal surfaces now call explicit coach-self `/v1` routes in API mode; they use API-owned simulated provider state for method create/list/default/delete and withdrawal request/cancel/complete, with no real money provider invoked
  ...
  - db-backed `POST /v1/uploads/init` now persists pending `MediaObject` and `UploadSession` records and returns signed private-bucket `PUT` targets; `POST /v1/uploads/:uploadSessionId/complete` is the backend finalization and scan-proof boundary before media can become available; seed mode keeps the placeholder upload URL only as a non-production compatibility path
  ...
  - child profile, injury, medical, emergency-contact, and consent records now live behind `/v1/athletes/*`; guardian sharing invite/list/accept/decline/cancel/remove now live behind `/v1/families*`, `/v1/me/guardian-invites`, and `/v1/guardian-invites*` in non-mock mode instead of local family account storage; the API now persists those surfaces through repository-backed storage instead of route-local memory, `injury-service.ts` fails closed instead of writing local injury records outside mock mode, and a narrow `ath_user*` compatibility bridge remains only for legacy seed/auth fixtures
  ...
  - repeat booking from a booking detail now reads `/v1/bookings/:bookingId/rebook-context` in non-mock mode, so the next booking draft is copied from backend-visible booking context instead of trusting local screen state
  - saved coach/favourite relationships now use `/v1/me/favourite-coaches*` in non-mock mode, with self-scoped backend persistence, audited save/remove deny paths, and local `favourites` storage retained only for mock/demo mode
  - multi-week package and initial recurring-plan creation in non-mock mode now use backend series authority through `/v1/booking-series`, which writes `RecurringSeries` plus linked `Booking` rows with `recurringSeriesId` and `seriesIndex`; series list/detail/cancel/pause/resume/update also use backend authority, backend booking completion now writes attendance proof records plus progress-visible `SessionNote` proof when a coach completion note is present and drives recurring-series derived partial/completed state, local booking-series mirrors are mock-only, update/reschedule changes only existing future linked bookings while backend-syncing mutable linked invoices, and settled linked invoices block series changes until explicit invoice/refund adjustment authority handles them
  - group-session list/detail/create/publish/cancel/register/roster/attendance flows in non-mock mode now run through `/v1/group-sessions*` and `/v1/group-session-registrations*`; club-linked group-session create requires an active club role that can create organisation sessions, assigned-coach creates require `assign_session_coach` and an active club-staff target, squad-scoped creates must use a squad owned by the supplied club, and the group-session services no longer depend on removed `/api/group-sessions*`, `/api/registrations*`, or training-session fan-out paths
  - group-session registration is backend-denied unless the session is `PUBLISHED` or `FULL`, so cancelled/draft/completed sessions cannot create fresh registrations, bookings, or invoices through direct API calls
  ...
  - group-session cancellation now fans out through the same backend authority: active registrations and linked bookings are cancelled, open linked invoices/payment attempts are voided/cancelled, paid linked invoices block until backend refund approval, counters are reset, and session attendance proof is detached from the live session
  ...
  - group-session attendance marking and clearing now write backend audit events with session, athlete, date, and resulting registration status metadata
  ...
  - `/v1/athletes/:athleteId/goals` now reads `Goal` and `GoalMilestone` through Prisma in real `db` mode, uses the DB fixture store only for fixture-backed tests, and remains protected by the athlete health read gate plus sensitive-read audit. The Prisma seed importer now carries goals and milestones so linked test users retain development data after DB import.
  - athlete analytics, skill history, goal mutations, skill updates, and coach analytics do not yet have `/v1` authorities. The analytics services now fail closed in non-mock mode instead of calling legacy `/api` routes or silently accepting no-op writes.
  ...
  - event list/create/detail/update/status/invite-club, event RSVP submission, event-scoped RSVP reads, RSVP reminder fan-out, event attendance reads/stats, and event check-in writes now use `GET/POST /v1/clubs/:clubId/events`, `GET/PATCH /v1/events/:eventId`, `POST /v1/events/:eventId/invites/club`, `/v1/events/:eventId/rsvp`, `GET /v1/events/:eventId/rsvps`, `GET /v1/events/:eventId/rsvps/:userId`, `POST /v1/events/:eventId/rsvps/remind`, `GET /v1/events/:eventId/attendance`, `GET /v1/events/:eventId/attendance/:userId`, `GET /v1/events/:eventId/attendance/stats`, `POST /v1/events/:eventId/checkins`, and `DELETE /v1/events/:eventId/checkins/:userId` in non-mock mode; `db` mode reads/writes `ClubEvent`, reads/upserts `EventRsvp`, queues event invite/reminder `Notification` rows, reads/writes event presence through `EventAttendance`, derives stats from `EventRsvp`/`EventAttendance`, and audits allow/deny paths. Academy read flows are compatibility aliases over club authority; academy-specific mutations remain fail-closed unless they become distinct club-backed `/v1` contracts.
  ...
  - org staffing console reads now use audited `GET /v1/clubs/:clubId/staffing-console` in non-mock mode, projecting active club staff and assigned club group-session workload from backend session/booking truth; existing club group-session delivery can be reassigned through audited `PATCH /v1/clubs/:clubId/work-assignments/:assignmentId`, which updates the group session and linked booking delivery coach, while true unassigned work still needs a backend assignment model or nullable delivery coach semantics
  - head-coach oversight reads now use audited `GET /v1/clubs/:clubId/head-coach/oversight` in non-mock mode, projecting club staff health and awaiting-completion queues from backend club, squad, group-session, booking, and participant truth; owner/admin reads see club scope, head-coach reads are scoped to assigned squads, and head-coach task/standard mutations remain fail-closed until backend persistence exists
  - owner dashboard reads now use audited `GET /v1/clubs/:clubId/owner-dashboard` in non-mock mode, composing governed staffing, head-coach oversight, invoice totals, and unresolved booking-linked trust incidents from backend truth; provider payouts remain simulated by design
  - booking report-problem submissions now create backend trust incidents through `POST /v1/safeguarding/incidents` in non-mock mode for every category, with safety issues tagged as `booking_issue_safety`; local `PROBLEM_REPORTS` writes are mock-only
  ...
  - seed-only verification/library routes are no longer public: `/v1/meta/seed-health` is privileged-admin-only, `/v1/drills` requires authenticated coach self-read or privileged admin access, and both remain disabled outside the seed backend
  - top-level staff post creation, post comment list/create/delete/reaction toggles, community group message send/read transitions, direct/thread message send/delete, notification read/dismiss/clear state, and notification preferences are backend-authoritative in API mode: active staff scope, readable-post scope, active group/thread membership, sender ownership, or notification ownership is required, the actor comes from auth, and local feed/comment/message/read/notification preference overlays are mock-only for these transitions
...
- Dedicated `Club Schedule` and `Team Schedule` routes now exist in the app.
  ...
  - match fixture create/read/status/result writes now use `ClubMatch` through `/v1/clubs/:clubId/matches` and `/v1/matches/:matchId/*`; player invite fan-out, availability responses, and lineup selection now use `ClubMatchPlayer` through `/v1/matches/:matchId/players/invite`, `/v1/matches/:matchId/players/respond`, and `/v1/matches/:matchId/lineup`
