...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - multi-week package creation in non-mock mode now uses backend series authority through `POST /v1/booking-series`, which writes `RecurringSeries` plus linked `Booking` rows with `recurringSeriesId` and `seriesIndex`; recurring plan management still fails closed until its own backend authority path is implemented
