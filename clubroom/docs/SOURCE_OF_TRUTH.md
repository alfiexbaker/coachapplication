...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - coach profile in non-mock mode now reads its own offerings from `/v1/coaches/me/offerings`, booking session-type selection reads target coach active offerings from `/v1/coaches/:coachId/offerings`, and go-live state writes through `PATCH /v1/auth/me` instead of local-only toggles
