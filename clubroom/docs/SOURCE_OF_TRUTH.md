...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - `/v1/community-groups`, `/v1/posts`, `/v1/posts/:postId/comments`, `/v1/comments/:commentId`, `/v1/message-threads`, and `/v1/me/notifications` now resolve through one shared community/media repository in both seed and `db` modes instead of route-local marketplace seed-table reads
  ...
  - real `db` mode no longer falls back to marketplace seed rows for `/v1/invites*`; outside the API test db-fixture fallback, these routes return `503` until the invite repository is implemented against Prisma
  ...
  - top-level staff post creation, post comment list/create/delete, community group message send/read transitions, direct/thread message send/delete, notification read/dismiss/clear state, and notification preferences are backend-authoritative in API mode: active staff scope, readable-post scope, active group/thread membership, sender ownership, or notification ownership is required, the actor comes from auth, and local feed/comment/message/read/notification preference overlays are mock-only for these transitions; comment likes fail closed in API mode until a backend comment-reaction model exists
