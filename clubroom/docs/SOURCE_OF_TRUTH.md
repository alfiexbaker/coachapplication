...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - community group message send/read transitions and notification read/dismiss/clear state are backend-authoritative in API mode: active group membership or notification ownership is required, the actor comes from auth, and local message/read/notification state overlays are mock-only for these transitions
