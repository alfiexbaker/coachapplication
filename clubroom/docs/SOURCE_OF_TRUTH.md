...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - community group message send/read transitions are backend-authoritative in API mode: active group membership is required, the sender/reader comes from auth, and local message/read overlays are mock-only
