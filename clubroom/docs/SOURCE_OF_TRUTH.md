...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - org staffing console and club work reassignment are mock-only until backend assignment authority exists; non-mock mode fails closed instead of reading or mutating local `SESSION_OFFERINGS` / booking mirrors for staff assignment truth
