...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - top-level staff post creation, post comment list/create/delete, community group message send/read transitions, notification read/dismiss/clear state, and notification preferences are backend-authoritative in API mode: active staff scope, readable-post scope, active group membership, or notification ownership is required, the actor comes from auth, and local feed/comment/message/read/notification preference overlays are mock-only for these transitions; comment likes fail closed in API mode until a backend comment-reaction model exists
