...
- The biggest production seams still not finished are broader grant coverage, live payment-provider cutover, and the remaining non-mock app cutover needed for full db-backed release behavior:
  ...
  - invoice reconciler transitions now run through the shared invoice runtime in seed, db-fixture, and db modes; linked invoices must still resolve to their authoritative booking before payment sessions, provider confirmation, or manual money-state changes proceed
  ...
  - multi-week package and initial recurring-plan creation in non-mock mode now use backend series authority through `/v1/booking-series`, which writes `RecurringSeries` plus linked `Booking` rows with `recurringSeriesId` and `seriesIndex`; series list/detail/cancel also use backend authority and local booking-series mirrors are mock-only
