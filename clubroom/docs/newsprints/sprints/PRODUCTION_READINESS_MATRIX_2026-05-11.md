...
| Reviews/favourites/rebook            | Booking-linked review/rebook services                  | Partial; `POST /v1/bookings/:bookingId/reviews` now accepts only completed bookings from the booked guardian/athlete, replays duplicate submits, stores verified review proof, audits success/deny paths, and the review screen submits through this authority in non-mock mode | Coach profile review read linkup to `/v1`, no local review mirror needed for profile proof, and rebook context authority                                   |
