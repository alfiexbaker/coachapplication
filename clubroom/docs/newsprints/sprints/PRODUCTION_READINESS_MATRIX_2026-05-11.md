...
| Direct bookings                      | `/v1/bookings*`                                        | Create/list/detail db-backed; lifecycle has idempotency, conflict/version checks, status-event audit, and db-fixture parent/coach/deny proof | Backend series/package authority, payment/refund hard wall, broader mirror cleanup                            |
| Recurring/multi-week/block packaging | Booking package/series authority                       | Non-mock local package writes now fail closed instead of creating fake booking success                                                       | Define backend packaging model and `/v1` series authority                                                     |
...
| Invoices/payments/refunds            | `/v1/invoices*`, payment attempts                      | DB-fixture invoice generation and manual money transitions link back to authoritative booking state; hosted provider remains simulated       | Live Stripe boundary decision, refund SMS/2FA hard wall                                                       |
...
3. Booking create/list/detail/cancel/reopen now have db-fixture proof that a parent can book a child, the coach can see it, unrelated parent/coach/athlete actors are denied, and booking mirrors are not client-local API-mode authority; remaining booking risk is backend series/package implementation, refund hard wall, and broader mirror cleanup.
4. Multi-week and recurring flows now fail closed in non-mock mode instead of using local `saveBookingDirect` success; backend series/package authority still needs design and implementation.
5. Video/media must not become `AVAILABLE` until upload finalization and malware scanning are enforced.
6. Invoice manual transitions now run through the db-aware invoice runtime and require authoritative booking linkage; refunds still need SMS/2FA hard wall before launch.
