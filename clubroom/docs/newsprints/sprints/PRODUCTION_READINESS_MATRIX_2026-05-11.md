...
| Families, athletes, guardians        | `/v1/families*`, `/v1/athletes*`                       | Core implemented; guardian invite/cancel/remove writes now run through backend family authority with audited allow/deny paths and same-payload invite replay                                                                                                             | Guardian invite acceptance/revocation inbox completion, broader versioning, no silent save failures           |
...
8. Guardian sharing invite/cancel/remove now has production backend authority; remaining launch risk is invite acceptance/decline inbox completion and broader guardian permission versioning.
