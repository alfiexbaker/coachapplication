...
| Reviews/favourites/rebook            | Booking-linked review/rebook services                  | Partial; `POST /v1/bookings/:bookingId/reviews` now accepts only completed bookings from the booked guardian/athlete, replays duplicate submits, stores verified review proof, and audits success/deny paths | Review screen/profile read linkup to `/v1`, no local booking read needed for proof, and rebook context authority                                   |
