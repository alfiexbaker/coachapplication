...
| 3     | Discover to trusted booking              | Parent, athlete, coach                | User finds a nearby trusted provider and reaches a real booking decision.                    | discover service, coach profile, availability slots, `/v1/bookings`                  | `app/discover/map.tsx`, `app/book-coach.tsx`, `app/coach/[id].tsx`, `app/book/[coachId]/*` | Booking session-type selection now reads target coach active offerings from `/v1/coaches/:coachId/offerings`; discover/profile surfaces still touch `SESSION_OFFERINGS`. Direct confirmation is gated by `/v1/bookings` success.                                              | Map/profile/session-type/review agree on offer, price, slot, eligibility, and backend bookability.                               |
