...
| Recurring/multi-week/block packaging | `/v1/booking-series` plus future recurring-plan routes | Multi-week package create/list/detail/cancel/pause/resume now use backend series authority in non-mock mode and no longer write local booking mirrors                                                       | Add update/reschedule authority once slot and money effects are explicit                                      |
...
4. Multi-week package create/list/detail/cancel/pause/resume now runs through `/v1/booking-series*` in non-mock mode, writes backend `RecurringSeries` and linked bookings, and keeps local mirrors non-authoritative; recurring update/reschedule remains deferred until slot and money effects are explicit.
