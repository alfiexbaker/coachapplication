...
| 12    | Operational communication                | Club staff, coach, parent, athlete    | Staff-led feed, messages, notifications keep people coordinated.                             | `/v1/posts`, `/v1/posts/:postId/comments`, `/v1/message-threads`, `/v1/me/notifications` | feed, club posts, profile posts, chat, notifications                                       | Reads are db-aware; staff-only post creation, post comment create/delete, group message send/read, direct/thread message send/delete, notification state, and preferences are backend-owned; remaining risk is comment reactions and richer moderation policy. | Staff top-level posting only; comments controlled; message/notification state is relationship-scoped and reliable.               |
...
| Session invites                      | `/v1/invites*`                                         | Scaffolded; direct invite create now has same-key idempotency replay/conflict, direct invite acceptance creates bookings through backend booking repository authority in db-fixture mode, terminal responses replay safely, accept/decline flips after response are denied, and create/cancel/remind/dismiss/respond write paths emit allowed and denied audit events. Recurring invite partial acceptance and invite RSVP state now fail closed outside mock mode instead of writing local invite/RSVP mirrors. Real db mode returns `503` instead of falling back to marketplace seed rows until the invite repository is Prisma-backed. | Full invite repository extraction, backend recurring-invite semantics, backend RSVP decision or launch demotion, broader idempotency coverage for remaining writes, and UI mirror cleanup |
...
| Feed/posts/comments                  | Community/media APIs                                   | Post reads plus staff-only top-level post creation and post comment list/create/delete now run through the shared backend community/media repository in seed and `db` modes; post/comment authors come from auth, active staff/readable-post scope is enforced, unsafe media posts require backend upload proof, one-level replies and idempotency conflicts are tested, and comment likes fail closed outside mock until a comment-reaction model exists. | Controlled comment policy/versioning and backend comment reactions                                    |
...
10. Staff-only top-level post creation, post comment create/delete, community group message send/read writes, direct/thread message send/delete, and notification read/dismiss/clear/preference state are backend-owned in API mode; remaining community/messaging/notification write risk is comment reactions and richer moderation policy/versioning.
