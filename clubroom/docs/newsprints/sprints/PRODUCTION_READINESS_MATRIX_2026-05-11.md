...
| Session invites                      | `/v1/invites*`                                         | Scaffolded; direct invite acceptance now creates bookings through backend booking repository authority in db mode; create/cancel/remind/dismiss/respond write paths now emit allowed and denied audit events | Full invite repository extraction, broader idempotency coverage for every invite write, and UI mirror cleanup |
...
| Attendance/completion                | Booking/group delivery APIs                            | Booking completion writes backend attendance proof and session-note proof when the coach supplies a completion note; group attendance mark/clear writes audit events                                         | Group-session cancel fan-out and broader proof/media linkage                                                  |
...
1. Session invite acceptance now creates authoritative db-mode bookings through the backend booking repository, and create/cancel/remind/dismiss/respond writes now emit allowed/denied audit events; remaining invite risk is full invite repository ownership plus broader idempotency coverage across every invite write.
