...
| Messaging/notifications              | Community/media APIs                                   | Message-thread reads plus group message send/read receipts now use backend authority with active group membership gates, auth-derived sender/reader, idempotent message create replay/conflict, and allowed/denied audit events; notification read/dismiss/clear mutations are backend-owner scoped and audited | Message delete/direct-message send and notification preference mutations backend-owned or deferred            |
...
10. Community group message send/read writes plus notification read/dismiss/clear state are backend-owned in API mode; remaining community/messaging/notification write risk is feed/comment writes, message delete/direct-message send, and notification preference mutations.
