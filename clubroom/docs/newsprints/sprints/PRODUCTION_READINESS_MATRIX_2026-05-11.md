...
| Direct bookings                      | `/v1/bookings*`                                        | Create/list/detail db-backed; lifecycle has idempotency, conflict/version checks, status-event audit, and db-fixture parent/coach/deny proof                                                               | Recurring plan authority, payment/refund hard wall, broader mirror cleanup                                    |
| Recurring/multi-week/block packaging | `/v1/booking-series` plus future recurring-plan routes | Multi-week package create now uses backend series authority in non-mock mode and no longer writes local booking mirrors; recurring plan CRUD still fails closed outside mock mode                         | Add recurring plan authority and series lifecycle operations                                                  |
...
3. Booking create/list/detail/cancel/reopen now have db-fixture proof that a parent can book a child, the coach can see it, unrelated parent/coach/athlete actors are denied, and booking mirrors are not client-local API-mode authority; remaining booking risk is recurring plan authority, refund hard wall, and broader mirror cleanup.
4. Multi-week package create now runs through `POST /v1/booking-series` in non-mock mode, writes backend `RecurringSeries` and linked bookings, and keeps local mirrors non-authoritative; recurring plan CRUD still fails closed outside mock mode.
