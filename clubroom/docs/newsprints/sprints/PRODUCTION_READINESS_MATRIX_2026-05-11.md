...
| Direct bookings                      | `/v1/bookings*`                                        | Create/list/detail db-backed; lifecycle has idempotency, conflict/version checks, and status-event audit | Coach-visible db-mode proof, recurring/multi-week booking, money transition linkage, mirror cleanup           |
...
3. Booking cancel/reopen now have idempotency keys, conflict/version checks, transactional persistence, status-event audit, and denial/idempotency tests; remaining booking risk is coach-visible db-mode proof, recurring/multi-week booking, money transition linkage, and local mirror cleanup.
