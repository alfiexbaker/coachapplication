...
- `npm run api:dev:staging` / `npm --prefix apps/api run dev:staging` starts Fastify with `.env.staging.local` without file watch mode, so the API binds `API_HOST=0.0.0.0` and exposes the same LAN URL configured for the API-mode app without depending on local watcher limits.
- `npm --prefix apps/api run dev:staging:watch` remains available as an opt-in reload command for active API editing when the local OS watcher limit can support it.
...
- The old watcher-based staging startup could hit `EMFILE`; the canonical staging command is now non-watch, with watch mode kept as an explicit opt-in for active API editing.
...
| 3     | Discover to trusted booking              | Parent, athlete, coach                | User finds a nearby trusted provider and reaches a real booking decision.                    | discover service, coach profile, availability slots, `/v1/bookings`                  | `app/discover/map.tsx`, `app/book-coach.tsx`, `app/coach/[id].tsx`, `app/book/[coachId]/*` | Booking session-type/schedule, public coach profiles, and Discover Map booking fast-track now read target coach active offerings from `/v1/coaches/:coachId/offerings`; Discover Map coach-card pricing/bookability summaries use `/v1/coaches/offerings`; broader session-listing/org surfaces still touch `SESSION_OFFERINGS`. Direct confirmation is gated by `/v1/bookings` success. | Map/profile/session-type/review agree on offer, price, slot, eligibility, and backend bookability.                               |
...
| 11    | Club operating loop                      | Club admin, coach, parent, compliance | Club can manage staff, squads, schedule, updates, paid activity, evidence.                   | `contracts/club-governance.ts`, `/v1/clubs*`, club schedule service                  | club hub, settings, schedule, squads, invites                                              | Club create/memberships/squads planned; org staffing reads and existing group-session delivery reassignment now use governed `/v1` authority; true unassigned-work assignment semantics still need a backend model; legacy `club-service.ts` has high `/api/*` debt. | Club operations use capabilities, not ad-hoc roles; staff assignment and sensitive visibility are auditable.                     |
| 12    | Operational communication                | Club staff, coach, parent, athlete    | Staff-led feed, messages, notifications keep people coordinated.                             | `/v1/posts`, `/v1/posts/:postId/comments`, `/v1/comments/:commentId/reactions/toggle`, `/v1/message-threads`, `/v1/me/notifications` | feed, club posts, profile posts, chat, notifications                                       | Reads are db-aware; staff-only post creation, post comment create/delete/reaction toggles, group message send/read, direct/thread message send/delete, notification state, and preferences are backend-owned; remaining risk is richer moderation policy/versioning. | Staff top-level posting only; comments controlled; message/notification state is relationship-scoped and reliable.               |
...
| Session invites                      | `/v1/invites*`                                         | Scaffolded; direct invite create now has same-key idempotency replay/conflict, direct invite acceptance creates bookings through backend booking repository authority in db mode, terminal responses replay safely, accept/decline flips after response are denied, and create/cancel/remind/dismiss/respond write paths emit allowed and denied audit events. Recurring invite partial acceptance and invite RSVP state now fail closed outside mock mode instead of writing local invite/RSVP mirrors. Real db mode no longer falls back to marketplace seed rows; the current route adapter persists `Invite`, `InviteTarget`, and create-idempotency state through Prisma. | Dedicated invite repository extraction, backend recurring-invite semantics, backend RSVP decision or launch demotion, broader idempotency coverage for remaining writes, cross-resource transaction tightening, and UI mirror cleanup |
...
| Clubs, memberships, squads           | `/v1/clubs*`, governance contract                      | Partial; org staffing console and club work reassignment are mock-only and fail closed in API mode instead of treating local `SESSION_OFFERINGS` / booking mirrors as staff assignment truth.              | Club create, member ops, squads, branding/dashboard authority, backend staff assignment route/audit; no legacy `club-service.ts` `/api` operations  |
...
| Feed/posts/comments                  | Community/media APIs                                   | Post reads plus staff-only top-level post creation and post comment list/create/delete/reaction toggles now run through the shared backend community/media repository in seed and `db` modes; post/comment actors come from auth, active staff/readable-post scope is enforced, unsafe media posts require backend upload proof, one-level replies and idempotency conflicts are tested, deleted comments reject reactions, and allowed/denied comment writes are audited. | Controlled comment policy/versioning                                    |
| Messaging/notifications              | Community/media APIs                                   | Message-thread reads plus group message send/read receipts, direct/thread message send, and message delete now use backend authority with active group/thread membership gates, auth-derived sender/reader, sender/admin delete scope, idempotent message create replay/conflict, and allowed/denied audit events; notification read/dismiss/clear and preference mutations are backend-owner scoped and audited | Richer moderation policy/versioning and any deferred thread creation semantics                                                 |
...
7. Club schedule, events/RSVP, coach verification documents, and club admin operations still need backend authority or launch demotion; local org-staffing assignment now fails closed in API mode instead of mutating fake assignment truth.
...
10. Staff-only top-level post creation, post comment create/delete/reaction toggles, community group message send/read writes, direct/thread message send/delete, and notification read/dismiss/clear/preference state are backend-owned in API mode; remaining community/messaging/notification write risk is richer moderation policy/versioning.
