...
10. Staff-only top-level post creation, post comment create/delete, community group message send/read writes, and notification read/dismiss/clear/preference state are backend-owned in API mode; remaining community/messaging/notification write risk is comment reactions, message delete, and direct-message send.
