...
| 9     | Development proof, video, review, rebook | Parent, athlete, coach                | Delivery becomes visible progress, media proof, review, and repeat booking.                  | progress service, `/v1/athletes/:id/progress`, `/v1/videos*`, booking-linked reviews | development routes, videos, review, profile proof                                          | Video authority is strong; progress reads are now db-aware and completion notes feed progress, and review submission now has completed-booking API authority, but review UI/profile reads still keep local mirrors.                         | Proof is session-linked; private notes do not leak; reviews are booking-linked; rebook preserves context.                        |
...
| Reviews/favourites/rebook            | Booking-linked review/rebook services                  | Partial; `POST /v1/bookings/:bookingId/reviews` now accepts only completed bookings from the booked guardian/athlete, replays duplicate submits, stores verified review proof, audits success/deny paths, the review screen submits through this authority in non-mock mode, coach profile review tabs read verified reviews from `/v1/coaches/:coachId/reviews`, repeat booking drafts read `/v1/bookings/:bookingId/rebook-context` before prefill, and saved-coach relationships use `/v1/me/favourite-coaches*` in non-mock mode | No local review mirror needed for remaining profile proof and broader profile/storefront proof cleanup                                   |
