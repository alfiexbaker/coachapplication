...
| FM-7.2 | Reduce remaining non-decision alert popups | PARTIAL | YES | `npm run audit:alerts` => `uiFeedback.alert: 96` (down from 534 baseline) after converting 27 non-decision popup callsites to toast/direct flow | Strict WS3 pass to keep only true decision dialogs |
...
- `npm run audit:alerts` => PASS (`native Alert: 0`, `uiFeedback.alert: 96`, `prompt: 1`, `showToast: 394`)
