...
4. Booking create/list/detail/cancel/reopen are improved with db-fixture parent/coach/deny proof; multi-week package plus initial recurring-plan creation/list/detail/cancel/pause/resume/update now use backend series authority, invoice money transitions now require authoritative booking linkage, and legacy earnings payment/refund writes fail closed outside mock. Invite acceptance, broader recurring generation/session-completion semantics, media scan enforcement, guardian sharing, health/injury linkup, club admin operations, and community writes remain the highest-risk gaps.
...
3. Continue `PROD-API-02` by deciding the next backend-authoritative booking slice: either recurring generation/session-completion semantics, or Stripe-ready payment/refund endpoints.
