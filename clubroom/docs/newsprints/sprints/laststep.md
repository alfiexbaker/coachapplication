...
4. Booking create/list/detail/cancel/reopen are improved with db-fixture parent/coach/deny proof, but invite acceptance, recurring/multi-week booking, money transitions/refunds, media scan enforcement, guardian sharing, health/injury linkup, club admin operations, and community writes remain the highest-risk gaps.
...
3. Continue `PROD-API-02` by removing the remaining non-mock booking mirror risk from recurring/multi-week flows and linking money transition proof to authoritative booking state.
