...
4. Booking create/list/detail/cancel/reopen are improved with db-fixture parent/coach/deny proof; multi-week package plus initial recurring-plan creation/list/detail/cancel now use backend series authority, invoice money transitions now require authoritative booking linkage, and legacy earnings payment/refund writes fail closed outside mock. Invite acceptance, dedicated recurring pause/resume/update semantics, refund hard wall, media scan enforcement, guardian sharing, health/injury linkup, club admin operations, and community writes remain the highest-risk gaps.
...
3. Continue `PROD-API-02` by implementing backend cancellation/refund hard-wall behavior in the invoice runtime, then decide whether recurring pause/resume/update need dedicated server semantics before Stripe is introduced.
