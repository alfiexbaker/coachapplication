...
4. Booking create/list/detail/cancel/reopen are improved with db-fixture parent/coach/deny proof; recurring/multi-week local success now fails closed in non-mock mode, and invoice money transitions now require authoritative booking linkage. Invite acceptance, backend series authority, refund hard wall, media scan enforcement, guardian sharing, health/injury linkup, club admin operations, and community writes remain the highest-risk gaps.
...
3. Continue `PROD-API-02` by designing the backend series/package authority for recurring and multi-week bookings, then connect cancellation/refund hard-wall behavior to the invoice runtime.
