...
- Coach review legacy read-model keys are temporarily local compatibility mirrors in API mode. Booking-linked review submission uses `/v1/bookings/:bookingId/reviews`, and coach profile review tabs read verified public reviews from `/v1/coaches/:coachId/reviews`.
