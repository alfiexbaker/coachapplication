...
- Session completion no longer reads or writes legacy `session_offerings` storage in API mode. Individual booking completion uses `/v1/bookings/:bookingId/complete`; group attendance uses `/v1/group-session-registrations/:registrationId/attendance`; completed-booking `COACH_SESSIONS` ingestion and progress-report `COACH_SESSIONS` reads remain mock/demo-only. Individual booking detailed feedback opens a backend `/v1/session-feedback` draft and the per-athlete development feedback editor loads/saves `/v1/session-feedback` in API mode before any mock-only `COACH_SESSIONS` branch. Session-completion group/parent message shortcuts also fail closed before local `MESSAGES` reads/writes in API mode until the completion flow maps to real `/v1/message-threads/:threadId/messages` thread ids.
