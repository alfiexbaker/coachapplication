...
- Coach review legacy read-model keys are temporarily local compatibility mirrors in API mode. Booking-linked review submission itself uses `/v1/bookings/:bookingId/reviews`, and the mirror remains only until coach profile review tabs finish their `/v1` read linkup.
