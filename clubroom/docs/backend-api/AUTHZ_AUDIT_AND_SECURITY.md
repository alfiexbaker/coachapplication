...
- Supabase public tables have RLS enabled, direct `anon`/`authenticated`/`PUBLIC` table, sequence, and function privileges revoked, and `SECURITY DEFINER` functions kept out of the exposed `public` schema. Product data access goes through the Fastify `/v1` API unless a route explicitly adds reviewed RLS policies and grants. The Supabase Data API is disabled for public clients, and `scripts/db-staging-preflight.js` verifies that control on every strict run by calling `/rest/v1/` with an active publishable key and requiring Supabase's exact secret/service-role-only rejection. Migration `20260803195500_data_api_empty_schema` applies Supabase's supported disabled-Data-API workaround: PostgREST's `authenticator` role is pointed at a locked, empty `pgrst_no_exposed_schemas` schema so schema-cache refreshes stop emitting missing-schema errors without exposing product tables. Before intentionally re-enabling the Data API, reset `authenticator`'s `pgrst.db_schemas` role setting and reload PostgREST. A missing probe, an unexpected response, re-enabled public access, any direct client/public grant, disabled RLS, or a public security-definer function remains a release blocker. The 25 effective `supabase_admin` future-object defaults remain visible as warnings only while that live public-client rejection is proven; they are not claimed revoked. On 2026-08-03 the account-owner dashboard exposed the recommended `Automatically expose new tables` control after connector reauthentication, but saving it disabled again returned a Supabase-side HTTP 500. The Data API was immediately restored to disabled and the strict public-client probe now passes with 61/61 migrations applied. Apply `scripts/sql/revoke-supabase-default-grants.sql` from a context with `supabase_admin` authority, or retry the owner control after Supabase resolves that platform failure, then rerun the preflight.
...
- Do not put raw sensitive free-text into audit or application-log metadata. Use stable ids, field names, categories, booleans, lengths, counts, or redacted fingerprints instead.
