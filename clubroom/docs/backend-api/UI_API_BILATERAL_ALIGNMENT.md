...
| Spine | UI route(s) | Main component(s) | Frontend service(s) | API endpoint(s) | Tables | Authz/Grant rules | Audit events | Notes |
|---|---|---|---|---|---|---|---|---|
| Booking | `app/book/[coachId]/schedule.tsx` | `components/ui/booking/*` | `services/booking-service.ts`, `services/availability-service.ts` | `/v1/bookings/*` | `bookings`, `booking_participants`, `booking_status_events` | parent/athlete/coach + idempotency + version checks | `booking.created`, `booking.cancelled` | map current wizard steps and review/confirm behavior |
