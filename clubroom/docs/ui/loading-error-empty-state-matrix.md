...
- `UI-LOAD-08` ops work covered manage bookings, head-coach oversight, club settings, and club setup completion with `useScreen` keyed section-skeleton loading, retryable errors, retained refresh behavior, and shared event-driven reloads where relevant instead of bespoke `setLoading`/`setRefreshing` state machines.
- `UI-LOAD-08` redirect and submit-only cleanup covered availability and club create by removing fake async `useScreen` loads that only rendered theme colors, while the manage redirect now has a real retryable resolver instead of unused manual loading state.
- `UI-LOAD-08` finance routes now retain keyed payment and invoice frames: earnings, invoice list, and invoice detail declare section-skeleton data keys, and earnings keeps its header chrome mounted for loading and error states.
- `UI-LOAD-08` is now closed: settings, admin invite codes, invoices, earnings, manage, availability, club setup, and club settings routes have explicit route ownership; settings/admin form routes use keyed `useScreen` frames, retryable first-load failures, stable shells, refresh-preserving frames, and action-level submit progress instead of spinner-only or blank fallback states.
