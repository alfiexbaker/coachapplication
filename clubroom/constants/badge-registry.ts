...
export const SKILL_BADGES: BadgeDefinition[] = [
  ...
  },
  {
    ...
    tone: "default",
    description:
      "Recognised for intelligent runs and positioning off the ball.",
    category: "technical",
    tier: 1,
    pointValue: 10,
  },
...
export const MEANINGFUL_SKILL_BADGE_IDS = new Set(
  SKILL_BADGES.flatMap((b) =>
    !b.id.startsWith("badge_recognition_") ? [b.id] : [],
  ),
);

/** Quick lookup: badge ID → BadgeDefinition */
export const SKILL_BADGE_BY_ID = new Map(SKILL_BADGES.map((b) => [b.id, b]));
