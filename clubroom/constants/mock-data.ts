...
export const coachParentSessions: Record<string, string[]> = {
...
export function getCoachesForParent(parentId: string): string[] {
  return Object.entries(coachParentSessions)
...
const personalFeedSeedPosts: ClubFeedPost[] = [
  {
    id: 'personal_post_1',
    clubId: 'club_lions',
...
clubFeedPosts.push(...personalFeedSeedPosts);

/**
 * Get personal feed posts for a specific coach.
 * Returns posts where feedType is 'PERSONAL' or 'BOTH' and the author matches.
 */
export function getPersonalFeedForCoach(coachId: string): ClubFeedPost[] {
  return clubFeedPosts
    .filter(
      (post) =>
...
export function getCombinedFeedForParent(
  ...
  const personalPosts: AggregatedFeedPost[] = clubFeedPosts
    .filter(
      (post) =>
        coachIds.includes(post.authorId || '') &&
        (post.feedType === 'PERSONAL' || post.feedType === 'BOTH') &&
  ...
  if (filter && filter !== 'all') {
    ...
    return [...clubPosts, ...filteredPersonal].sort(
      (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
  }

  return [...clubPosts, ...personalPosts].sort(
    (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
  );
}
