...
export default function SessionInviteDetailScreen() {
  ...
  const {
    ...
    load: async () => {
      ...
      return data
        ? ok(data)
        : err(serviceError("NOT_FOUND", "Invite not found"));
    },
    deps: [id],
    events: [
  ...
  const invite =
    loadedInvite ?? (id ? (inviteDetailSnapshots.get(id) ?? null) : null);
  useEffect(() => {
    if (loadedInvite && id) {
  ...
  const [currentRsvpStatus, setCurrentRsvpStatus] = useState<
    "going" | "maybe" | "cant_go" | null
  >(null);
  const isCoach = currentUser?.role === "COACH";
  const isOwner = invite?.coachId === currentUser?.id;
  const isRecipient = invite?.parentId === currentUser?.id;
  const coachName = invite ? getSessionInviteCoachName(invite) : "Coach";
  const athleteNames = invite ? getSessionInviteAthleteNames(invite) : [];
  const derivedStatus = (() => {
  ...
  const totalRsvpResponses =
    rsvpCounts.going + rsvpCounts.maybe + rsvpCounts.cantGo;
  useEffect(() => {
      ...
      async () => {
        ...
        if (!result.success) {
          uiFeedback.showToast(
            result.error?.message ?? "Could not create the booking.",
            "error",
          );
          return;
        }
        ...
        if (acceptedInvite.bookingId) {
          router.replace(
            Routes.booking(acceptedInvite.bookingId, {
  ...
  const handleRsvp = async (rs: "going" | "maybe" | "cant_go") => {
  ...
  const handleSendReminder = async () => {
    ...
    uiFeedback.showToast(
      "We've nudged the parent to respond to this invite.",
      "success",
    );
  };
  ...
  return renderShell(
    <>
          ...
          {totalRsvpResponses > 0 && (
            <Animated.View entering={FadeInDown.delay(170).springify()}>
              <SurfaceCard style={s.att}>
                <AvatarStack
                  attendees={rsvpResponses.flatMap((r) =>
                    r.status === "going"
                      ? [
                          {
                            id: r.userId,
      ...
      <PaymentModal
        ...
        invite={invite}
        selectedSlot={
          selectedSlot !== null ? invite.proposedSlots[selectedSlot] : null
        }
      />
    </>,
  );
