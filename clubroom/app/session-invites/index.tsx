...
export default function SessionInvitesScreen() {
      ...
      [
        ...
        { text: 'Decline', style: 'destructive', onPress: async () => {
          try {
            ...
            onRefresh();
          } catch { Alert.alert('Error', 'Failed to decline invite. Please try again.'); }
        }},
      ],
    );
  }, [onRefresh]);
      ...
      [
        ...
        { text: 'Cancel Invite', style: 'destructive', onPress: async () => {
          try {
            ...
            onRefresh();
          } catch { Alert.alert('Error', 'Failed to cancel invite. Please try again.'); }
        }},
      ],
    );
  }, [onRefresh]);

  const handleDismissInvite = useCallback(async (invite: SessionInvite) => {
    Alert.alert(
      'Remove Invite',
      ...
      [
        ...
        { text: 'Remove', style: 'destructive', onPress: async () => {
          try {
            ...
            onRefresh();
          } catch { Alert.alert('Error', 'Failed to remove invite. Please try again.'); }
        }},
      ],
    );
  }, [onRefresh]);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
        ...
        <Row gap="sm" align="center" style={styles.headerTitle}>
          ...
          {pendingCount > 0 && (
            <View style={[styles.badge, { backgroundColor: colors.error }]}>
              <ThemedText style={[styles.badgeText, { color: colors.onPrimary }]}>{pendingCount}</ThemedText>
            </View>
          )}
        </Row>
        {userIsCoach && (
      ...
      {showFilterChips && <InviteStatusFilter filter={filter} pendingCount={pendingCount} onChangeFilter={handleChangeFilter} />}
          ...
          <EmptyState
            ...
            title={
              filter === 'pending' ? 'No pending invites'
                : filter === 'responded' ? 'No responded invites'
                : mode === 'sent' ? 'No invites sent'
                : 'No invites received'
            }
...
const styles = StyleSheet.create({
  ...
  headerTitle: { flex: 1 },
  badge: {
    minWidth: 20, height: 20, borderRadius: Radii.md,
  ...
  pendingBanner: {
    padding: Spacing.md, borderRadius: Radii.md, marginBottom: Spacing.md,
  },
