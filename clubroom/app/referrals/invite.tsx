  ...
  if (c.status === 'error') {
    return (
      ...
      >
        ...
        <ErrorState
          message={c.error?.message || 'Unable to load your referral code.'}
          onRetry={c.retry}
        />
      </SafeAreaView>
    );
        ...
        <>
          ...
          <Animated.View entering={FadeInDown.delay(150).springify()}>
            ...
            <ThemedText style={[styles.subheadline, { color: palette.muted }]}>
              Share your code with friends. When they sign up and complete their first booking, you
              both earn wallet credits!
            </ThemedText>
          </Animated.View>
          ...
          <Animated.View entering={FadeInDown.delay(200).springify()}>
            <SurfaceCard style={styles.codeCard}>
              ...
              <Row gap="sm" style={styles.codeActions}>
                <Clickable
                  onPress={c.handleCopyCode}
                  style={[
                    styles.actionButton,
                    {
                      backgroundColor: c.copied
                ...
                >
                  <Row align="center" gap="xxs">
                    <Ionicons
                      name={c.copied ? 'checkmark' : 'copy-outline'}
                      size={18}
                      color={c.copied ? palette.success : palette.icon}
                    />
                    <ThemedText
                      style={[
                        styles.actionButtonText,
                        { color: c.copied ? palette.success : palette.text },
                      ]}
                    >
                ...
                <Clickable
                  onPress={c.handleCopyLink}
                  style={[
                    styles.actionButton,
                    {
                      backgroundColor: c.linkCopied
                        ? withAlpha(palette.success, 0.09)
                        : palette.background,
                ...
                >
                  <Row align="center" gap="xxs">
                    <Ionicons
                      name={c.linkCopied ? 'checkmark' : 'link-outline'}
                      size={18}
                      color={c.linkCopied ? palette.success : palette.icon}
                    />
                    <ThemedText
                      style={[
                        styles.actionButtonText,
                        { color: c.linkCopied ? palette.success : palette.text },
                      ]}
                    >
          ...
          <Animated.View entering={FadeInDown.delay(250).springify()}>
            ...
            <Row justify="center" gap="md" style={styles.shareOptions}>
              {(['share-social', 'chatbubble', 'mail', 'ellipsis-horizontal'] as const).map(
                (icon) => (
                  <ShareButton
                    key={icon}
          ...
          <Animated.View entering={FadeInUp.delay(350).springify()}>
            <View style={styles.termsContainer}>
              <ThemedText style={[styles.termsText, { color: palette.muted }]}>
                Credits are awarded after your friend completes their first booking. Credits expire
                12 months from the date they&apos;re earned.
              </ThemedText>
            </View>
          </Animated.View>
        </>
