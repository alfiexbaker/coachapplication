...
export default function AllBadgesScreen() {
  ...
  if (!c.currentUser)
    return (
      <EmptyState
        icon="person-outline"
        title="Not logged in"
        message="Please log in to view your badges"
      />
    );
  ...
  return (
    ...
    >
      <BadgeStats
        totalBadges={c.stats.total}
        unlockedBadges={c.stats.unlocked}
        totalPoints={c.stats.points}
      />
        ...
        >
          {FILTER_TABS.map((tab) => {
            ...
            return (
              <Clickable
                key={tab.key}
                onPress={() => c.handleFilterChange(tab.key)}
                style={
                  [
                    styles.filterTab,
                    isActive && {
              ...
              >
                <Row align="center" gap="xxs">
                  ...
                  />
                  <ThemedText
                    style={[styles.filterLabel, { color: isActive ? palette.tint : palette.muted }]}
                  >
                    {tab.label}
                  </ThemedText>
                  <View
      ...
      {c.loading ? (
        ...
        <SurfaceCard style={styles.centeredCard}>
          ...
          <ThemedText type="defaultSemiBold" style={styles.emptyTitle}>
            {c.activeFilter === 'unlocked'
              ? 'No badges earned yet'
              : c.activeFilter === 'in-progress'
        ...
        SECTION_ORDER.filter((section) => c.filteredBySection.has(section)).map((section) => (
          <BadgeSectionGrid
            key={section}
            sectionKey={section}
            badges={c.filteredBySection.get(section) || []}
            onBadgePress={c.handleBadgePress}
          />
        ))
      )}
