...
} from "@/hooks/use-media-gallery";
import {
  PhotoViewer,
  type PhotoViewerItem,
} from "@/components/progress/photo-viewer";
...
export default function MediaGalleryScreen() {
  ...
  const handlePressItem = (item: MediaGalleryItem) => {
    if (item.type === "photo") {
  ...
  const renderGalleryItem = ({ item }: { item: MediaGalleryItem }) => (
    <Clickable
      style={[
        styles.tile,
        {
        ...
        <>
          ...
          <Row
            style={[
              ...
              {
                backgroundColor: withAlpha(colors.text, 0.64),
              },
            ]}
          >
            <ThemedText
  ...
  return (
    ...
    >
      ...
      {status === "error" ? (
        <ErrorState
          message={error?.message ?? "Unable to load media gallery."}
          onRetry={retry}
        />
      ) : null}
