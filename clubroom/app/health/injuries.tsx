...
const FILTERS: { value: StatusFilter; label: string; colorKey: 'text' | 'error' | 'warning' | 'success' }[] = [
...
const EMPTY_ICONS: Record<StatusFilter, string> = {
  ALL: 'medical-outline', ACTIVE: 'pulse-outline', RECOVERING: 'trending-up-outline', HEALED: 'checkmark-circle-outline',
};
              ...
              return (
                <Clickable key={filter.value} onPress={() => c.handleFilterChange(filter.value)}>
                  <Row align="center" gap="xs" style={[styles.filterTab, {
                    ...
                    <ThemedText style={[styles.filterLabel, { color: isActive ? palette.onPrimary : palette.text }]}>{filter.label}</ThemedText>
                    <View style={[styles.filterCount, { backgroundColor: isActive ? withAlpha(palette.onPrimary, 0.2) : palette.border }]}>
                      <ThemedText style={[styles.filterCountText, { color: isActive ? palette.onPrimary : palette.muted }]}>
        ...
        {c.loading ? (
          ...
          <Animated.View entering={FadeInDown.springify()} style={styles.emptyState}>
            <View style={[styles.emptyIcon, { backgroundColor: withAlpha(palette.tint, 0.09) }]}>
              <Ionicons name={EMPTY_ICONS[c.statusFilter] as keyof typeof Ionicons.glyphMap} size={48} color={palette.tint} />
            </View>
            <ThemedText type="subtitle" style={styles.emptyTitle}>
              {c.statusFilter === 'ALL' ? 'No Injuries' : `No ${c.statusFilter.charAt(0) + c.statusFilter.slice(1).toLowerCase()} Injuries`}
            </ThemedText>
            <ThemedText style={[styles.emptyText, { color: palette.muted }]}>
              {c.statusFilter === 'ALL' ? "You haven't logged any injuries yet." : `You don't have any ${c.statusFilter.toLowerCase()} injuries.`}
            </ThemedText>
          </Animated.View>
        ) : (
