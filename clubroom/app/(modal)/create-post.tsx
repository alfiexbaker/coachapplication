  ...
  if (p.isCoach && !p.membership?.clubId) {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: palette.background }]} edges={['top', 'bottom']}>
        <CreatePostForm
          title={p.title} body={p.body} postType={p.postType} imageUri={p.imageUri}
          eventDate={p.eventDate} eventLocation={p.eventLocation} showDatePicker={p.showDatePicker}
          isPosting={p.isPosting} canPost={p.canPost} onClose={handleClose}
          onTitleChange={p.setTitle} onBodyChange={p.setBody} onPostTypeChange={p.handlePostTypeChange}
          onPickImage={p.pickImage} onRemoveImage={p.removeImage} onPost={p.handlePersonalPost}
          onSetEventDate={p.setEventDate} onSetEventLocation={p.setEventLocation}
          onOpenDatePicker={() => p.setShowDatePicker(true)} onCloseDatePicker={() => p.setShowDatePicker(false)}
        />
      </SafeAreaView>
    );
  }
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: palette.background }]} edges={['top', 'bottom']}>
        ...
        <View style={styles.content}>
          <SurfaceCard style={styles.card}>
            ...
            <ThemedText type="subtitle" style={styles.cardTitle}>Join a Club to Post</ThemedText>
