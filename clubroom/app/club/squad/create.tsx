...
export default function CreateSquadScreen() {
  ...
  if (status === 'error') {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
        <ErrorState message={error?.message || 'Failed to open squad creation flow.'} onRetry={retry} />
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <Row align="center" justify="space-between" style={styles.header}>
          ...
          <ThemedText type="title" style={Typography.heading}>Create Squad</ThemedText>
          <View style={{ width: 24 }} />
        </Row>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              {AGE_GROUPS.map((option) => (
                <Clickable
                  ...
                  style={[styles.optionChip, {
                    backgroundColor: selectedAgeGroup?.label === option.label ? colors.tint : colors.surface,
                    borderColor: selectedAgeGroup?.label === option.label ? colors.tint : colors.border,
                  }]}
                  onPress={() => setSelectedAgeGroup(option)}
                >
                  <ThemedText style={[Typography.bodySmallSemiBold, { color: selectedAgeGroup?.label === option.label ? colors.onPrimary : colors.text }]}>
                    {option.label}
                  </ThemedText>
                </Clickable>
              ))}
              ...
              {SQUAD_LEVELS.map((level) => (
                <Clickable
                  ...
                  style={[styles.levelOption, {
                    ...
                    backgroundColor: selectedLevel === level ? withAlpha(colors.tint, 0.06) : 'transparent',
                  }]}
                  onPress={() => setSelectedLevel(level)}
                >
                  <Row gap="md" align="center">
                    <View style={[styles.radio, { borderColor: selectedLevel === level ? colors.tint : colors.border }]}>
                      {selectedLevel === level && <View style={[styles.radioInner, { backgroundColor: colors.tint }]} />}
                    </View>
                    <ThemedText type="defaultSemiBold">{level}</ThemedText>
                  </Row>
                </Clickable>
            ...
            <Row style={styles.optionsGrid}>
              {SKILL_TAGS.map((tag) => (
                <Clickable
                  key={tag}
                  style={[styles.optionChip, {
                    backgroundColor: selectedTags.includes(tag) ? withAlpha(colors.tint, 0.12) : colors.surface,
          ...
          <SurfaceCard style={styles.previewCard}>
            <ThemedText style={[Typography.caption, { color: colors.muted, textTransform: 'uppercase' }]}>Preview</ThemedText>
            <Row gap="md" align="center">
                ...
                {selectedTags.length > 0 && (
                  <Row gap="xs" style={{ marginTop: Spacing.xxs }}>
                    {selectedTags.map((tag) => (
                      <View key={tag} style={[styles.previewTag, { backgroundColor: withAlpha(colors.tint, 0.06) }]}>
                        <ThemedText style={{ color: colors.tint, ...Typography.caption }}>{tag}</ThemedText>
                      </View>
                    ))}
                  </Row>
                )}
        ...
        <View style={[styles.footer, { borderTopColor: colors.border }]}>
          <Clickable
            style={[styles.createButton, { backgroundColor: isValid ? colors.tint : colors.border }]}
