      ...
      if (targetAudience === 'SQUADS' && selectedSquadIds.length > 0 && publish) {
        ...
        const result = await bulkInviteService.inviteSquadsToEvent({
          event: {
            ...
            location: isVirtual ? 'Online' : venue,
            startDate: new Date(`${date}T${startTime}:00`).toISOString(),
            endDate: endTime ? new Date(`${date}T${endTime}:00`).toISOString() : undefined,
            squadIds: selectedSquadIds,
            allClub: false,
            maxParticipants: maxAttendees ? parseInt(maxAttendees, 10) : undefined,
