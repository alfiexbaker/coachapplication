  ...
  return (
    ...
    >
      <Row align="center" gap="xxs" style={styles.statHeader}>
        <Ionicons
          name={consentService.getConsentIcon(type) as keyof typeof Ionicons.glyphMap}
          size={18}
...
export default function ConsentsScreen() {
  ...
  const renderItem = ({ item, index }: { item: AthleteConsent; index: number }) => (
    <Animated.View entering={FadeInDown.delay(index * 30).springify()}>
      <ConsentCard
        athleteConsent={item}
        onPress={() => router.push(Routes.rosterAthlete(item.athleteId))}
      />
    </Animated.View>
  );
              ...
              {row.map((type) => {
                ...
                return (
                  <StatCard
                    ...
                    denied={stat.denied}
                    isActive={c.selectedType === type}
                    onPress={() => c.handleStatCardPress(type)}
                  />
                );
              })}
            </Row>
      ...
      <Row gap="sm" style={styles.searchSection}>
        ...
        >
          <Ionicons
            ...
            size={20}
            color={c.activeFiltersCount > 0 ? palette.onPrimary : palette.text}
          />
