...
export default function RosterScreen() {
  ...
  const [selectedAthleteForRemoval, setSelectedAthleteForRemoval] = useState<RosterEntry | null>(
    null,
  );
  ...
  const handleConfirmRemoval = useCallback(
    async (reason: RemovalReason, customReason?: string, archive?: boolean) => {
      ...
      try {
        ...
        showUndoToast(`${removedAthleteName} removed from roster`, async () => {
          try {
            await rosterService.undoRemoval(coachId, record.id);
            retry();
  ...
  const handleModalConfirm = useCallback(
    (reason: string, customReason?: string, archive?: boolean) => {
  ...
  if (status === 'error')
    return <ErrorState message={error?.message ?? 'Failed to load roster'} onRetry={retry} />;
          ...
          rightElement={
            <Row gap="md" align="center">
              <Clickable
                ...
                hitSlop={8}
                accessibilityLabel={isSelecting ? 'Cancel selection' : 'Select athletes'}
                accessibilityRole="button"
              >
                <Ionicons
                  name={isSelecting ? 'close' : 'checkbox-outline'}
