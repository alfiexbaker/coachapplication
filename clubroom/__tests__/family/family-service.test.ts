...
describe('Family Service', () => {
  ...
  describe('getFamilySpending', () => {
    test('should return spending breakdown per child', async () => {
      const spending = await familyService.getFamilySpending('parent1');
      ...
      spending.forEach((childSpending) => {
        ...
        assert.ok(typeof childSpending.averagePerSession === 'number');
      });
    });

    test('should calculate average per session correctly', async () => {
      const spending = await familyService.getFamilySpending('parent1');
      ...
      spending.forEach((childSpending) => {
        if (childSpending.sessionCount > 0) {
          ...
          assert.strictEqual(
            childSpending.averagePerSession.toFixed(2),
            expectedAverage.toFixed(2)
          );
        } else {
          assert.strictEqual(childSpending.averagePerSession, 0);
        }
      });
    });

    test('should include monthly breakdown', async () => {
      const spending = await familyService.getFamilySpending('parent1');

      spending.forEach((childSpending) => {
        if (childSpending.monthlyBreakdown) {
          childSpending.monthlyBreakdown.forEach((month) => {
            assert.ok(month.month.match(/^\d{4}-\d{2}$/), 'Month should be in YYYY-MM format');
            assert.ok(typeof month.amount === 'number');
            assert.ok(typeof month.sessionCount === 'number');
          });
        }
      });
    });

    test('should include trend information', async () => {
      const spending = await familyService.getFamilySpending('parent1');

      spending.forEach((childSpending) => {
        if (childSpending.trend) {
          assert.ok(['up', 'down', 'stable'].includes(childSpending.trend));
        }
        if (childSpending.trendPercent !== undefined) {
          assert.ok(typeof childSpending.trendPercent === 'number');
        }
      });
    });
  });
  ...
  describe('getFamilySpendingSummary', () => {
    test('should return spending summary with correct structure', async () => {
      const summary = await familyService.getFamilySpendingSummary('parent1');

      assert.ok(typeof summary.totalSpent === 'number');
      assert.ok(typeof summary.thisMonth === 'number');
      assert.ok(typeof summary.lastMonth === 'number');
      assert.ok(summary.currency);
      assert.ok(['up', 'down', 'stable'].includes(summary.trend));
      assert.ok(typeof summary.trendPercent === 'number');
    });
  });
